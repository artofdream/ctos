...
6. **Still Planned (at accept time).** Identity `.data` / heap tear. Superseded in part by [ADR-037](ADR-037-identity-data-tear.md) (`.data`/stacks) and [ADR-038](ADR-038-identity-heap-tear.md) (heap). PAN enable on `-cpu cortex-a57`; EL0 entry without `TLBI VMALLE1`; lower-EL IRQ while standing; umbrella EL0 isolation.
