...
6. **Still Planned.** Identity `.data` / heap tear (needs SP relocate + high allocator VAs). [ADR-031](ADR-031-track-a-leftovers.md) prints `ident: data-stay` / `ident: heap-stay` so that gap stays fail-closed; it does **not** yank those pages. PAN enable on `-cpu cortex-a57`; EL0 entry without `TLBI VMALLE1`; lower-EL IRQ while standing; umbrella EL0 isolation.
