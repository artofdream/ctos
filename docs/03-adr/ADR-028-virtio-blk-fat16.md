...
2. **virtio-mmio, modern (version 2).** Scan QEMU `virt` transports at `0x0a000000 + n×0x200` (32 slots) for Device ID 2. Negotiate `VIRTIO_F_VERSION_1`. One split virtqueue. DMA addresses are **identity PAs** of `.bss` buffers (`.data`/heap stay identity-mapped; [ADR-025](ADR-025-identity-rodata-tear.md)). Poll `used.idx` after `QueueNotify`. No virtio IRQ this mile. Device MMIO already sits in the Device-nGnRnE L1 block.
...
6. **Honesty.** Say “the guest programmed virtio-blk and read a known FAT16 file through the thin VFS” only when the serial / tests pass. Do **not** say: app hosting is done, POSIX, Linux VFS, FAT32, xv6, writeable volumes, virtio-net, “EL0 isolated,” or “secure OS.” Track A A8–A9 stay Planned.
