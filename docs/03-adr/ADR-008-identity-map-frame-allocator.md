...
2. **L1 blocks, not a full 4 KiB identity.** Entry 0 (`0x0`–`1 GiB`) is Device-nGnRnE + XN (UART, GIC, flash). Entry 1 (`0x4000_0000`–`0x7FFF_FFFF`) is Normal WB executable (RAM). A 1 GiB RAM block over-maps the 128 MiB guest; the allocator must not hand out frames past `RAM_BASE + 128 MiB`.
...
- This ADR does not claim Raspberry Pi, GICv3, EL0 user maps, or ASID isolation.
