...
- Status: Accepted (capability probe). Historical a57: `pan: absent`. Default smoke CPU since [ADR-079](ADR-079-pan-cpu-reopen.md): `-cpu cortex-a76` → `pan: present`. **PAN enable** + EL1-vs-EL0 fault: [ADR-080](ADR-080-pan-enable-fault.md) ([ADR-054](ADR-054-pan-enable-lock.md) gate items 3–4).
- Date: 2026-09-12
...
4. **When `n != 0`.** Serial `pan: present`. [ADR-080](ADR-080-pan-enable-fault.md) enables PSTATE.PAN and proves EL1-vs-EL0 fault. Smoke expects `pan: present` + `pan: enabled` + `pan: el1-fault` on `-cpu cortex-a76`.
