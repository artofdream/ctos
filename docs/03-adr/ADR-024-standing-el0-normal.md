...
2. **Supported path.** The A3 guest ELF loader maps `PT_LOAD`, then `install_task(e_entry, stack_top, user_ttbr0)` and `ERET`. The loaded image runs at EL0 until `SYS_EXIT`. `SYS_YIELD` / `SYS_UART_WRITE` stay at EL0 and must observe `is_active()` (`el0: task-active`). A2’s memcpy path stays so `libctos:*` markers remain.
