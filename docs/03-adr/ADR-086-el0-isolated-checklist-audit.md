...
| --- | --- | --- | --- |
| Agent-box default smoke | 2026-09-26 13:59–14:04 | `scripts/qemu-smoke.sh` on `915796f`, stock QEMU 10.0.13 (Debian), `virt -cpu cortex-a76` TCG, rustc 1.100.0-nightly `0fc141305` (repo pin; corrected in ADR-087 — this line first said `5ceaf6608`, the box's `+nightly`) | `qemu-smoke: ok`; force-fail exit 1 (fail-closed ok). All §A markers below present. `inject-nmi=>error:machine does not provide NMIs` → `el0: serror-park`. |
