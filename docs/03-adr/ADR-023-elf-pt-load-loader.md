...
5. **Image source.** First cut embedded the ELF (`include_bytes!`). [ADR-031](ADR-031-track-a-leftovers.md) reads the same bytes from FAT `/hello`. `build.rs` still compiles and publishes `target/hello-libctos.elf`. Not a second QEMU `-kernel`.
