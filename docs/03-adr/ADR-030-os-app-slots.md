...
6. **Performance.** Expected costs stay the documented shape (boot/load, SVC, ASID/TTBR). The new marker proves the path is measurable. A compared CNTPCT pair vs a probe-only linked-in trip is [ADR-046](ADR-046-slot-perf-delta.md). Do not invent a bench.
...
- Track A children A1–A9 now have a first cut. [ADR-032](ADR-032-track-a-leftovers.md) takes cross-update + embed-off + isolation honesty. Slot-load CNTPCT pair is [ADR-046](ADR-046-slot-perf-delta.md) (Verified measurement, not a percent). Remaining Planned / decided: PAN enable non-goal on default a57, never yank `_start`, leftover identity RAM optional, umbrella isolation Planned/non-claim ([ADR-047](ADR-047-isolation-leftovers-decisions.md)); product “app hosting is done” ([ADR-048](ADR-048-app-hosting-claim-criteria.md)).
