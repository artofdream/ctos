...
2. **2 MiB L2 blocks** where a block is entirely kernel-image or entirely frame-pool. **One L3 table per mixed 2 MiB** (originally the single block that straddled `__kernel_end`; [ADR-015](ADR-015-ro-nx-text-data.md) splits on `__data_start` and `KERNEL_TEXT`, which can be two blocks).
...
- `src/paging.rs` grows `L2_RAM` and a kernel L3 pool (one table per mixed 2 MiB). Identity VA == PA is unchanged.
- Linker-stack guard pages are [ADR-014](ADR-014-linker-stack-guard-pages.md). The RO+NX text/data split and `SCTLR.WXN` are [ADR-015](ADR-015-ro-nx-text-data.md).
