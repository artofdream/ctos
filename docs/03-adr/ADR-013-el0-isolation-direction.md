...
6. **Isolation remains Planned.** Missing: PAN (typically unimplemented on `-cpu cortex-a57` / ARMv8.0 — read `ID_AA64MMFR1_EL1.PAN` before claiming it), full TTBR1 / higher-half identity teardown ([ADR-016](ADR-016-ttbr1-private-page.md) is the private-page cut; [ADR-017](ADR-017-ttbr1-high-el1-exec.md) is the EL1-fetch cut — identity boot stub stays), EL0 entry without a full TLBI. Do not move FR IDs. Do not mint FR-16+. Do not claim PAN.
