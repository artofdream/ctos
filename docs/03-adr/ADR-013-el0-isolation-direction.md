...
6. **Isolation remains Planned.** Missing: PAN **enable** (typically unimplemented on `-cpu cortex-a57` / ARMv8.0 — [ADR-026](ADR-026-pan-capability.md) reads `ID_AA64MMFR1_EL1.PAN` and records absent; do not silent `-cpu` switch), full TTBR1 / higher-half identity teardown ([ADR-016](ADR-016-ttbr1-private-page.md) is the private-page cut; [ADR-017](ADR-017-ttbr1-high-el1-exec.md) is the EL1-fetch cut; [ADR-018](ADR-018-identity-teardown-first-cut.md) is the first identity-tear cut; [ADR-019](ADR-019-identity-text-range-tear.md) jumps the post-MMU continuation high and unmaps a 16 KiB dedicated text range; [ADR-020](ADR-020-identity-fnptr-reloc.md) rewrites rustc vtables and unmaps live identity `.text`; [ADR-025](ADR-025-identity-rodata-tear.md) unmaps identity `.rodata` — `.data`/heap stay), EL0 entry without a full TLBI. A documented SVC ABI is [ADR-021](ADR-021-svc-syscall-abi.md) (not isolation, not app hosting). The `libctos` CRT is [ADR-022](ADR-022-libctos-crt.md) (not isolation, not app hosting). Do not move FR IDs. Do not mint FR-16+. Do not claim PAN.
