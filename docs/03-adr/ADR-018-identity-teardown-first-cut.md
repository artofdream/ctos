...
- [ADR-019](ADR-019-identity-text-range-tear.md) jumps the post-MMU continuation to high VA and expands the dedicated tear to 16 KiB. [ADR-020](ADR-020-identity-fnptr-reloc.md) rewrites rustc vtables and unmaps live identity `.text`. `.rodata`/`.data`/heap stay.
