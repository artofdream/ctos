...
1. **Prior OS is `ba6541c`.** That is the A9 merge commit (`A9: Disconnect OS image from app payloads (ADR-030) (#60)`). No earlier `main` SHA has the slot/FAT path. Do not store a prior kernel ELF in git. `scripts/qemu-smoke.sh` builds that commit in a git worktree and QEMU `-kernel`s it with the **same** app bytes this tip published (sha256 pin via `sha256sum`, `shasum -a 256`, or `openssl dgst -sha256`). Docker smoke needs `git` in the image and `.git` in the build context — do not invent a stored kernel blob. Both boots must print `slot: fat` / `slot: mapped` / `slot: ok` and must not print `slot: embed` / `slot: probe missed`. This OS SHA must differ from `ba6541c`.
