...
4. **Not a guest loader.** `build.rs` compiles that program on the host and extracts `PT_LOAD` bytes at/after `EL0_PAGE`. The kernel copies the image onto the standing EL0 page and `ERET`s — same style as A1. Parsing ELF on the host is not A3.
5. **One page.** The image must fit in 3584 bytes so the stack can sit at `EL0_PAGE + 4096` like A1. rust-lld’s extra ELF-header `PT_LOAD` at `0x80000000` is discarded at extract time.
