...
- Status: Accepted (claim gate). Product “apps run independently / hosting done” is **Verified** when criteria 1–5 are Met ([ADR-052](ADR-052-sponsor-accept-app-hosting.md) records sponsor accept for #5). Do **not** claim Linux userspace, POSIX, containers, or “EL0 isolated.”
- Date: 2026-09-13

## Context

Track A children A1–A9 have first cuts (ABI, CRT, loader, standing-as-normal, isolation miles, memfs, FAT, recipes, OS/app slots + cross-update / embed-off). The honesty ledger and roadmap correctly refuse to round those miles up to “app hosting is done.” Sponsors still need **explicit criteria** so a Verified claim is fail-closed rather than mood.

Related: [ADR-030](ADR-030-os-app-slots.md), [ADR-032](ADR-032-track-a-leftovers.md), [ADR-029](ADR-029-containers-nongoal.md), [ADR-052](ADR-052-sponsor-accept-app-hosting.md) (sponsor accept), [hosting-apps.md](../overview/hosting-apps.md), [ADR-047](ADR-047-isolation-leftovers-decisions.md) (isolation leftovers are separate — closing them is **not** required to meet these criteria, and meeting these criteria does **not** make “EL0 isolated” true).

## Decision

### A1–A9 first cuts

Treat A1–A9 **first cuts** as **Verified** when the honesty ledger has matching rows for this tip (or cited merge tips). That is **not** product app hosting by itself.

### Product claim Verified only when all criteria below are met
...
5. **Explicit sponsor accept** — A PR that flips the product row to Verified must cite this ADR’s checklist and must not self-merge ([ADR-002](ADR-002-pr-identity-split.md)). Sponsor accept for the 2026-09-13 flip is recorded in [ADR-052](ADR-052-sponsor-accept-app-hosting.md).
...
## Accepted claim (amendment)

On tip `main` @ `be789c5`+ with [ADR-052](ADR-052-sponsor-accept-app-hosting.md): criteria **1–5 Met**. Product row **Verified** for the scoped freestanding slot-hosting sentence. Still refuse Linux/POSIX/containers/“EL0 isolated”/OTA/immutable-OS marketing.

## Honesty

Say “A1–A9 first cuts exist” when the ledger rows say so. Say the product claim is **Verified under ADR-048** only with the checklist Met×5 and sponsor accept cited. Do **not** say “apps update independently” as OTA, or “container host” — containers stay non-goal regardless.

## Consequences

- Ledger row “Track A app hosting (slots)” is **Verified** and cites this ADR + [ADR-052](ADR-052-sponsor-accept-app-hosting.md).
- [hosting-apps.md](../overview/hosting-apps.md), Track A / roadmap, NFR-10 text point here.
- No new FR/NFR IDs. Do not mint NFR-15+.

## Progress checklist

Living Met/Partial/Blocked table (tip evidence): [hosting-apps.md — Claim criteria](../overview/hosting-apps.md#claim-criteria-adr-048) and [honesty-ledger.md — ADR-048 progress](../framework/honesty-ledger.md#adr-048-progress-checklist). Product claim is **Verified** when criterion 5 is Met ([ADR-052](ADR-052-sponsor-accept-app-hosting.md)).
