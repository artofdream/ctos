...
- PAN, full higher-half identity teardown, and preemption remain later. Standing EL0 and the ASID-tagged TLB mile are [ADR-013](ADR-013-el0-isolation-direction.md). TTBR1 private page is [ADR-016](ADR-016-ttbr1-private-page.md). EL1 high-VA fetch is [ADR-017](ADR-017-ttbr1-high-el1-exec.md). The first identity-tear cut is [ADR-018](ADR-018-identity-teardown-first-cut.md). The identity `.text` range tear is [ADR-019](ADR-019-identity-text-range-tear.md). The live `.text` tear after a high-VA vtable rewrite is [ADR-020](ADR-020-identity-fnptr-reloc.md). The SVC ABI mile is [ADR-021](ADR-021-svc-syscall-abi.md). The `libctos` CRT mile is [ADR-022](ADR-022-libctos-crt.md). The guest ELF loader mile is [ADR-023](ADR-023-elf-pt-load-loader.md). Standing EL0 as normal mode is [ADR-024](ADR-024-standing-el0-normal.md) (app hosting Planned). Umbrella isolation still Planned. RO+NX text/data is [ADR-015](ADR-015-ro-nx-text-data.md). Stack guard pages are [ADR-014](ADR-014-linker-stack-guard-pages.md). EL0 first mile + user-TTBR0 read mile are also ADR-013.
