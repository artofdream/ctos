...
A9 (OS/app slot disconnect) sits **after** A1–A4. It does not override the list above. The first cut plus [ADR-032](../03-adr/ADR-032-track-a-leftovers.md) leftover mile: cross-update on `ba6541c`, no hello embed. Product “app hosting is done” is **Verified** under [ADR-048](../03-adr/ADR-048-app-hosting-claim-criteria.md) / [ADR-052](../03-adr/ADR-052-sponsor-accept-app-hosting.md) (not Linux/POSIX/containers).
...
| A5 | Isolation completion (remaining identity tear; PAN only if CPU + ADR) | **Identity `.rodata` / `.data` / heap tears Verified** on this branch `qemu-smoke` ([#36](https://github.com/artofdream/ctos/issues/36), [ADR-025](../03-adr/ADR-025-identity-rodata-tear.md) / [ADR-037](../03-adr/ADR-037-identity-data-tear.md) / [ADR-038](../03-adr/ADR-038-identity-heap-tear.md)). PAN **enable** is **non-goal (locked)** on default `-cpu cortex-a57` ([ADR-054](../03-adr/ADR-054-pan-enable-lock.md); `pan: absent`). Umbrella non-claim until checklist ([ADR-055](../03-adr/ADR-055-el0-isolated-checklist.md)). Not “EL0 isolated.” |
...
| A7 | virtio-blk + FAT16 | **block + FAT mile Verified** on this tip when the ledger has `blk: ok` / `fat: ok` ([#38](https://github.com/artofdream/ctos/issues/38), [ADR-028](../03-adr/ADR-028-virtio-blk-fat16.md)). Same VFS `open`. FAT write depth: [ADR-050](../03-adr/ADR-050-fat16-write.md). Root listing: [ADR-056](../03-adr/ADR-056-fat16-readdir.md). Root delete: [ADR-057](../03-adr/ADR-057-fat16-delete.md). Not POSIX. Not a Linux FS. |
...
| A9 | Disconnect OS image from app payloads | **First cut** ([#48](https://github.com/artofdream/ctos/issues/48), [ADR-030](../03-adr/ADR-030-os-app-slots.md)) + leftover ([ADR-032](../03-adr/ADR-032-track-a-leftovers.md)): OS ELF + published `hello-libctos.elf` + FAT `/hello`. A2–A4 load that file (no production embed). Cross-update: same ELF on this OS and `ba6541c` when qemu-smoke prints both `cross-update … slot:ok` lines. Slot-load CNTPCT pair ([ADR-046](../03-adr/ADR-046-slot-perf-delta.md)): `perf: app-load` + probe-only `perf: embed-load` + `perf: slot-delta` (QEMU TCG lab; not a percent). Not OTA. Product claim Verified under [ADR-048](../03-adr/ADR-048-app-hosting-claim-criteria.md) / [ADR-052](../03-adr/ADR-052-sponsor-accept-app-hosting.md) (not Linux/containers). |

## Out of scope for Track A

OCI/Docker containers (**non-goal**, [ADR-029](../03-adr/ADR-029-containers-nongoal.md)), glibc/musl ports, SMP, networking (Track N — [ADR-063](../03-adr/ADR-063-network-foundation-scope.md); Planned until N1). See [host-apps.md](../framework/host-apps.md).
