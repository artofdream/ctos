...
| A5 | Isolation completion (remaining identity tear; PAN only if CPU + ADR) | **Identity `.rodata` tear Verified** when serial / tests pass ([#36](https://github.com/artofdream/ctos/issues/36), [ADR-025](../03-adr/ADR-025-identity-rodata-tear.md)). PAN **enable** stays **Planned** with CPU evidence ([ADR-026](../03-adr/ADR-026-pan-capability.md): `pan: absent` on `-cpu cortex-a57`). Not “EL0 isolated.” A6–A9 stay **Planned**. |
