...
| P-SEC-3j | Identity `.rodata` / `.data` / heap tear | `.rodata` + `.data`/stacks unmapped + high-only access. Heap still Planned. | **`.rodata` Verified** (ADR-025). **`.data`/stacks Verified:** 2026-09-12 agent-box `qemu-smoke` (see honesty ledger; ADR-037). Heap stay (allocator still identity). Do not claim “the kernel moved.” |
...
| A5 | Isolation completion ([#36](https://github.com/artofdream/ctos/issues/36), ADR-025 / ADR-026 / ADR-032 / ADR-037) | `ident: rodata` + `ident: data` + `ident: heap-stay`; `pan: id=` + `pan: absent` | **`.rodata` + `.data` + PAN ID-field Verified** when probes pass. Heap stay + PAN enable **Planned**. Not “EL0 isolated.” |
