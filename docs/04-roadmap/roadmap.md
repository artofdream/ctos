...
| A5 | Isolation completion ([#36](https://github.com/artofdream/ctos/issues/36), ADR-025 / ADR-026 / ADR-032 / ADR-037 / ADR-038 / ADR-039 / ADR-040 / ADR-041) | `ident: rodata` + `ident: data` + `ident: heap`; `el0: no-vmalle1`; `el0: irq` + `el0: irq-default`; `pan: id=` + `pan: absent` | **`.rodata` + `.data` + heap + EL0 no-VMALLE1 + lower-EL IRQ + default IRQ-unmasked ERET + PAN ID-field Verified** on this branch `qemu-smoke`. PAN enable **Planned**. Not “EL0 isolated.” |
