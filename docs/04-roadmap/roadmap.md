...
| P-SEC-3d | Standing EL0 context | `el0: standing` + `el0: restored`; `is_active()` true only while standing | **Verified:** 2026-09-11 cloud `qemu-smoke` (see honesty ledger). Not POSIX. Default `ERET` still masks IRQ (ADR-040 probes unmasked). |
...
| P-SEC-3n | Lower-EL IRQ while standing (ADR-040) | `el0: irq`; `#[test_case]` `lower_el_irq_while_standing` | **Verified** when this tip’s `qemu-smoke` prints `el0: irq` (see honesty ledger). FIQ/SError still park. Default `ERET` still masks I. Not “EL0 isolated.” |
| P-SEC-3l | Umbrella EL0 isolation | Standing + PAN + full TTBR1 / identity teardown | **Planned.** Specific miles (P-SEC-3…P-SEC-3n) are not this row. Do not claim “EL0 isolated.” |
...
| A5 | Isolation completion ([#36](https://github.com/artofdream/ctos/issues/36), ADR-025 / ADR-026 / ADR-032 / ADR-037 / ADR-038 / ADR-039 / ADR-040) | `ident: rodata` + `ident: data` + `ident: heap`; `el0: no-vmalle1`; `el0: irq`; `pan: id=` + `pan: absent` | **`.rodata` + `.data` + heap + EL0 no-VMALLE1 + lower-EL IRQ while standing + PAN ID-field Verified** on this branch `qemu-smoke`. PAN enable **Planned**. Not “EL0 isolated.” |
