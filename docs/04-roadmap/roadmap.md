...
| A5 | Isolation completion ([#36](https://github.com/artofdream/ctos/issues/36), ADR-025 / ADR-026 / ADR-032 / ADR-037 / ADR-038) | `ident: rodata` + `ident: data` + `ident: heap`; `pan: id=` + `pan: absent` | **`.rodata` + `.data` + heap + PAN ID-field Verified** on this branch `qemu-smoke`. PAN enable **Planned**. Not “EL0 isolated.” |
