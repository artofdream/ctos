...
| P-SEC-3k | PAN on virt `cortex-a57` | `ID_AA64MMFR1_EL1.PAN != 0` **and** an EL1-vs-EL0 access fault | **Non-goal on default probe CPU** ([ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md) / ADR-026). ID-field Verified (`pan: absent`). Do not switch `-cpu` silently. Future CPU story needs its own ADR + sponsor. |
...
| P-SEC-3r | Taken lower-EL SError QMP attempt (ADR-045) | Guest A-clear path + `el0: serror-arm` + QMP `inject-nmi` log; `el0: serror-park` if not taken | **Deferred / non-goal** on this smoke machine ([ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md)). Guest A-clear = dormant prep; park Verified. Do not require `el0: serror`. Not “EL0 isolated.” |
| P-SEC-3l | Umbrella EL0 isolation | Standing + PAN + full TTBR1 / identity teardown | **Planned / non-claim** ([ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md)). Constraints: no PAN enable on a57, `_start` stays, taken SError deferred. Specific miles are not this row. Do not claim “EL0 isolated.” |
...
| A5 | Isolation completion ([#36](https://github.com/artofdream/ctos/issues/36), ADR-025 / ADR-026 / ADR-032 / ADR-037 / ADR-038 / ADR-039 / ADR-040 / ADR-041 / ADR-042 / ADR-043 / ADR-044 / ADR-045 / ADR-047 / ADR-049) | `ident: rodata` + `ident: data` + `ident: heap` + `ident: ram`; `el0: no-vmalle1`; `el0: irq` + `el0: irq-default` + `el0: fiq` + `el0: serror-park`; `pan: id=` + `pan: absent` | **`.rodata` + `.data` + heap + leftover RAM + EL0 no-VMALLE1 + lower-EL IRQ + default IRQ-unmasked ERET + lower-EL FIQ + SError park honesty + PAN ID-field Verified**. [ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md): taken SError deferred/non-goal; PAN enable non-goal on default a57; never yank `_start`; umbrella Planned/non-claim. [ADR-049](../03-adr/ADR-049-identity-ram-tear.md) takes optional leftover RAM. Not “EL0 isolated.” |
...
### Isolation leftovers (decided — [ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md))

- Taken SError: **deferred / non-goal** on this smoke machine until an honest inject exists. Park Verified; guest A-clear plumbing = dormant prep ([ADR-045](../03-adr/ADR-045-taken-serror-qmp.md)).
- PAN enable on default `-cpu cortex-a57`: **non-goal** (ID-field Verified `pan: absent`; do not switch `-cpu`). Future CPU = new ADR + sponsor.
...
- Umbrella “EL0 isolated”: remains **Planned / non-claim**.
