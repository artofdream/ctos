...
| P-SEC-3g | Identity-tear first cut (ADR-018) | `ident: split` + `ident: fault` + `ident: high` + `ident: no el0` + `ident: ok` | **This PR.** Full identity teardown / PAN / umbrella isolation stay **Planned**. Do not claim “the kernel moved.” |
