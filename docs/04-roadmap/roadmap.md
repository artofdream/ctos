...
| P-SEC-3p | Identity `_start` / leftover-RAM stay honesty (ADR-042) | `ident: start-stay` + `ident: ram-stay`; `#[test_case]` `identity_boot_stub_stays_while_live_torn` | **Verified** honesty. [ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md): **never yank `_start`**; leftover RAM optional Planned. Not “EL0 isolated.” |
...
- Yank `_start`: **never** while QEMU `-kernel` needs `0x4008_0000`. `ident: start-stay` Verified honesty. Leftover identity RAM after heap: **optional Planned** (not umbrella-required).
