...
| P-SEC-3r | Taken lower-EL SError QMP attempt (ADR-045) | Guest A-clear path + `el0: serror-arm` + QMP `inject-nmi` log; `el0: serror-park` if not taken | **Partial:** guest path wired; QEMU 10 virt+cortex-a57 `inject-nmi` fails (no TYPE_NMI). Taken stays **Planned**. Do not require `el0: serror`. Not “EL0 isolated.” |
...
### Isolation leftover (post ADR-045)

- Taken SError: [ADR-045](../03-adr/ADR-045-taken-serror-qmp.md) attempted ADR-044 H1; QEMU 10 virt+cortex-a57 has no working `inject-nmi`→SError. Guest A-clear path wired; taken still **Planned**; park stays Verified.
