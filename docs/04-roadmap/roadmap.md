...
| P-SEC-3l | Umbrella EL0 isolation | Standing + PAN + full TTBR1 / identity teardown | **Planned.** Specific miles (P-SEC-3…P-SEC-3m) are not this row. Do not claim “EL0 isolated.” |
...
| A5 | Isolation completion ([#36](https://github.com/artofdream/ctos/issues/36), ADR-025 / ADR-026 / ADR-032 / ADR-037 / ADR-038 / ADR-039) | `ident: rodata` + `ident: data` + `ident: heap`; `el0: no-vmalle1`; `pan: id=` + `pan: absent` | **`.rodata` + `.data` + heap + EL0 no-VMALLE1 + PAN ID-field Verified** on this branch `qemu-smoke`. PAN enable **Planned**. Not “EL0 isolated.” |
