...
| A5 | Isolation completion ([#36](https://github.com/artofdream/ctos/issues/36), ADR-025 / ADR-026 / ADR-032) | `ident: rodata` + `ident: data-stay` + `ident: heap-stay`; `pan: id=` + `pan: absent` | **`.rodata` + PAN ID-field Verified.** Stay markers + leftover honesty: see ledger. PAN enable / `.data`/heap tear **Planned**. Not “EL0 isolated.” |
