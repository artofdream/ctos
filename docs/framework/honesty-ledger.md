...
| Docker smoke (`Dockerfile` / `scripts/docker-smoke.sh`) | cts-ai Docker Desktop `linux/arm64`, 2026-09-09 | Build **Verified**; `docker run` **Failed** (three times); full smoke **Unknown** | (1) CRLF shebang → `exec ./scripts/qemu-smoke.sh: no such file or directory`. Ratchet: `.gitattributes` `*.sh`/`Dockerfile` `eol=lf`, image `sed`, `CMD ["bash","./scripts/qemu-smoke.sh"]`. (2) After LF, `linker \`cc\` not found` on `compiler_builtins`. Ratchet: `build-essential`. (3) After `cc`, `cargo build` succeeded; QEMU Failed: `failed to find romfile "efi-virtio.rom"` (hello missing, qemu exit 1). Ratchet: `qemu-efi-aarch64` + `ipxe-qemu` (noble Recommends skipped by `--no-install-recommends`); image `test -e /usr/lib/ipxe/qemu/efi-virtio.rom`. Full smoke after this third ratchet is **Unknown** until re-probed. This cloud VM still has no Docker engine. |
