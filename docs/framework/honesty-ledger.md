...
| Integration tests / QEMU test exit (M2) | `cargo +nightly test` → QEMU virt + `-semihosting`; `#[test_case]`; QEMU host exit 0 | Verified | 2026-09-08 cloud: `Running 2 tests` / `[ok]`. 2026-09-09 M3 re-probe: `Running 4 tests` / all `[ok]` (includes VBAR + BRK). |
...
| `scripts/qemu-smoke.sh` | Ran on this cloud VM | Verified | 2026-09-09 (`qemu-system-aarch64` 8.2.2, `rustc` 1.100.0-nightly `4aa1fbcf4`): hello (timeout 124), `exception: sync BRK` (`esr=0xf2000000`), `cargo test` 0 (4 cases), force-fail 1, `qemu-smoke: ok`. Not Docker. GHA on this PR is a separate row. |
