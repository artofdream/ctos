...
| Docker smoke (`Dockerfile` / `scripts/docker-smoke.sh`) | cts-ai Docker Desktop `linux/arm64`, 2026-09-09 | Build **Verified**; `docker run` **Failed** (twice); full smoke **Unknown** | (1) First `docker run` Failed: `exec ./scripts/qemu-smoke.sh: no such file or directory` (CRLF shebang). Ratchet: `.gitattributes` `*.sh`/`Dockerfile` `eol=lf`, image `sed` strip CR, `CMD ["bash","./scripts/qemu-smoke.sh"]`. (2) After LF, `docker run` got past shebang then Failed: `linker \`cc\` not found` compiling `compiler_builtins` (build-std). Ratchet: `build-essential` in the image. Full smoke after this second ratchet is **Unknown** until re-probed. This cloud VM still has no Docker engine. |
