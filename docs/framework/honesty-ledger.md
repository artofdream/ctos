...
| Docker smoke (`Dockerfile` / `scripts/docker-smoke.sh`) | cts-ai Docker Desktop `linux/arm64`, 2026-09-09 | Build **Verified**; full `docker run` **Failed** then **Unknown** | Sponsor: `docker build -t ctos-smoke .` succeeded. `docker run --rm ctos-smoke` failed: `exec ./scripts/qemu-smoke.sh: no such file or directory` (CRLF shebang from a Windows checkout). Ratchet: `.gitattributes` `*.sh`/`Dockerfile` `eol=lf`, image `sed` strip CR, `CMD ["bash","./scripts/qemu-smoke.sh"]`. Full smoke after the fix is **Unknown** until re-probed (sponsor may run `bash ./scripts/qemu-smoke.sh` locally). This cloud VM still has no Docker engine. |
