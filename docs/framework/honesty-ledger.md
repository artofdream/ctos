...
| CI on GitHub | Workflow file exists **and** a run is green | Unknown | `.github/workflows/smoke.yml` present (`ubuntu-24.04-arm` + `ubuntu-24.04`). No green run recorded on this revision yet. |
| Docker smoke (`Dockerfile` / `scripts/docker-smoke.sh`) | `docker build` + `docker run` executes `qemu-smoke.sh` | Unknown | Files present. This cloud VM had no Docker engine (`command -v docker` empty). linux/arm64 image unprobed. Do not copy an amd64-only success as arm64. |
