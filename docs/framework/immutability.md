...
**Today (first cut + leftover):** two host artifacts (kernel ELF + `target/hello-libctos.elf`) and a FAT16 `/hello` load path. A2–A4 load that file (no `include_bytes!`). Cross-update is the host two-boot probe on this OS and `ba6541c` ([ADR-031](../03-adr/ADR-031-track-a-leftovers.md)). Do not say “apps update independently.” Performance: `perf: app-load` is a measurement; a Verified *delta* vs the old embed stays Planned ([performance.md](performance.md#osapp-slot-disconnect-a9--expected-shape-not-a-bench)).
