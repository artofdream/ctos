...
`fs: create` / `fs: write` / `fs: read` / `fs: el0` / `fs: ok` stay fail-closed. A7 adds `blk: virtio` / `blk: cap` / `blk: rw` / `blk: ok` and `fat: mount` / `fat: read` / `fat: ok`. ADR-050 adds `fat: write` / `fat: rewrite` / `fat: create`. ADR-064 adds `fat: grow`. ADR-056 adds `fat: readdir` / `fat: entries`. ADR-057 adds `fat: delete`. ADR-073 adds `fat: mkdir`. ADR-075 adds EL0 `fs_mkdir` / `libctos: mkdir-ok` / `mkdemo: ok`. ADR-058 adds `vfs: mount` / `vfs: mounts`.
