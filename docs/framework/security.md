...
| virtio-net (N1 ARP + N2 ICMP + N3 UDP + N3.x/N4 EL0 SVC) | Guest programs a virtio-mmio net DMA master for ARP, ICMP echo, then one UDP datagram vs QEMU SLIRP. EL0 uses tiny SVCs only (kernel owns NIC). Not a BSD sockets / TCP product. | `src/virtio.rs` + `src/syscall.rs` ([ADR-066](../03-adr/ADR-066-virtio-net-first-frame.md), [ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md), [ADR-070](../03-adr/ADR-070-n3-udp-transport.md), [ADR-071](../03-adr/ADR-071-n3x-el0-udp-svc.md), [ADR-068](../03-adr/ADR-068-el0-net-svc-sample.md)) |
