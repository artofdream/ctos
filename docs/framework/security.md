...
| Identity `.data` / stacks | Identity `.data`/`.bss`/linker stacks unmapped after SP relocate + pointer rewrite. High twin stays. Heap stays. Not a relocated kernel. | `[__data_start, __kernel_end)` ([ADR-037](../03-adr/ADR-037-identity-data-tear.md)) |
...
| Remaining isolation gaps | Shared boot-stub text in the user table (handler must fetch if VBAR were still identity). No PAN enable on cortex-a57. EL0 trampoline still `TLBI VMALLE1`. Identity heap still live (`ident: heap-stay`). Lower-EL IRQ still parks. | User TTBR0 + ASID + TTBR1 + torn live `.text` + torn `.rodata` + torn `.data`/stacks + heap stay + SVC ABI; isolation Planned |
...
- Not secure boot, PAN enable, or a fully torn-down identity map (ADR-020/025/037 unmap live `.text` / `.rodata` / `.data`+stacks; heap stays — `ident: heap-stay`; `_start` stays at `0x4008_0000`).
...
| Identity `.data` / stack tear | Serial `ident: data` / `ident: data-fault` / `ident: data-high` | `.data`/stacks cut ([ADR-037](../03-adr/ADR-037-identity-data-tear.md)). Heap stays. |
| Identity heap still mapped | Serial `ident: heap-stay` | Honesty marker. Heap tear Planned. |
