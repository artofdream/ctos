...
Version: **v1.45** (2026-09-16). Slice/update of v1.44 (#115 ADR-069). N3 minimal UDP transport ([ADR-070](../03-adr/ADR-070-n3-udp-transport.md): `net: udp-tx` / `net: udp-rx` / `net: udp-ok` via SLIRP DNS `10.0.2.3:53` probe bait). Keep N1/N2/N4 + ADR-069 CNTPCT. Still **not** BSD sockets / TCP product / DHCP/DNS product / Wi‑Fi / “has networking / sockets OS.” ADR-068/067/066/063/069 unchanged on prior miles. ADR-062/061/060/053/054/055 locks unchanged. Product freestanding app-hosting claim remains **Verified** under ADR-048/052. Still not Linux/POSIX/`mount`/`unlink`/`getdents`/containers/preemption/exFAT. Not a v2 model and not “secure.”
...
| **DMA / virtio devices** | **Named; bounded** | A7 programs virtio-mmio blk (one queue, poll `used.idx`, identity-PA DMA). N1–N3 program virtio-mmio net (RX+TX queues, same poll/DMA pattern) for ARP, ICMP echo, then one UDP datagram ([ADR-066](../03-adr/ADR-066-virtio-net-first-frame.md) / [ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md) / [ADR-070](../03-adr/ADR-070-n3-udp-transport.md)). QEMU is still TCB. A malicious virtio device is out (hostile hypervisor). No BSD sockets / TCP product. |
...
| **Network attacker** | **Named; bounded (link only)** | N1–N3 attach QEMU user netdev + guest virtio-net for ARP + ICMP + one UDP datagram ([ADR-066](../03-adr/ADR-066-virtio-net-first-frame.md) / [ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md) / [ADR-070](../03-adr/ADR-070-n3-udp-transport.md)). No BSD sockets / TCP product. QEMU SLIRP is still TCB. Hostile on-wire peers beyond this probe are out of product scope. |
...
| virtio-net ARP (N1) + ICMP ping (N2) + UDP (N3) | Serial `net: virtio` / `net: mac` / `net: tx` / `net: rx` / `net: ok` + `net: icmp-tx` / `net: icmp-rx` / `net: ping-ok` + `net: udp-tx` / `net: udp-rx` / `net: udp-ok` | [ADR-066](../03-adr/ADR-066-virtio-net-first-frame.md) / [ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md) / [ADR-070](../03-adr/ADR-070-n3-udp-transport.md). ARP + ICMP + one UDP vs QEMU SLIRP only. Not BSD sockets / TCP. Host `-netdev` alone is not the probe. |
...
| virtio-blk + FAT16 | Serial `blk: ok` / `fat: ok` / `fat: write` / `fat: create` / `fat: grow` / `fat: readdir` / `fat: entries` / `fat: delete`; VFS `/probe` | block + FAT mile ([ADR-028](../03-adr/ADR-028-virtio-blk-fat16.md)) + write depth ([ADR-050](../03-adr/ADR-050-fat16-write.md)) + multi-cluster grow ([ADR-064](../03-adr/ADR-064-fat16-multi-cluster-grow.md)) + readdir ([ADR-056](../03-adr/ADR-056-fat16-readdir.md)) + delete ([ADR-057](../03-adr/ADR-057-fat16-delete.md)). Host `-drive` alone is not the probe. Not POSIX `getdents`/`unlink`/write API. |
