...
Version: **v1.29** (2026-09-13). Slice/update of v1.28 (product freestanding app-hosting claim **Verified** under [ADR-048](../03-adr/ADR-048-app-hosting-claim-criteria.md) / [ADR-052](../03-adr/ADR-052-sponsor-accept-app-hosting.md)). Taken SError = deferred/non-goal on this smoke machine; PAN enable = non-goal on default cortex-a57; never yank `_start`; leftover identity RAM after the heap is torn (`ident: ram*`); umbrella “EL0 isolated” stays Planned/non-claim. Still not Linux/POSIX/containers. Not a v2 model and not “secure.”
