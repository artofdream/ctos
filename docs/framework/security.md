...
Version: **v1.57** (2026-09-19). Slice/update of v1.56 (#128 ADR-081). Sponsor chose B1; [ADR-082](../03-adr/ADR-082-b1-qemu-type-nmi.md) research: no shippable upstream/apt QEMU pin makes `inject-nmi` deliver taken SError on virt TCG (master still lacks `TYPE_NMI`→async SError). Blocker: wait upstream or reviewed pinned build. Park Verified; do not require `el0: serror`. Keep PAN enable + N1–N5 + samples. Still **not** “EL0 isolated” / BSD sockets / Linux/containers. Not a v2 model and not “secure.”
