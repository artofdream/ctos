# Security — threat model v1.66 (NFR-10 / ADR-011 / ADR-012 / ADR-013 / ADR-014 / ADR-015 / ADR-016 / ADR-017 / ADR-018 / ADR-019 / ADR-020 / ADR-021 / ADR-022 / ADR-023 / ADR-024 / ADR-025 / ADR-026 / ADR-027 / ADR-028 / ADR-030 / ADR-032 / ADR-037 / ADR-038 / ADR-039 / ADR-040 / ADR-041 / ADR-042 / ADR-043 / ADR-044 / ADR-045 / ADR-047 / ADR-048 / ADR-049 / ADR-050 / ADR-052 / ADR-053 / ADR-054 / ADR-055 / ADR-056 / ADR-057 / ADR-058 / ADR-059 / ADR-060 / ADR-061 / ADR-062 / ADR-063 / ADR-064 / ADR-065 / ADR-066 / ADR-067 / ADR-068 / ADR-069 / ADR-070 / ADR-071 / ADR-072 / ADR-073 / ADR-074 / ADR-075 / ADR-076 / ADR-077 / ADR-078 / ADR-079 / ADR-080 / ADR-081 / ADR-082 / [ADR-083](../03-adr/ADR-083-b1-qemu-nmi-pin.md) / [ADR-084](../03-adr/ADR-084-b2-free-runner-serror.md) / [ADR-085](../03-adr/ADR-085-b2p-graviton-kvm-serror.md) / [ADR-086](../03-adr/ADR-086-el0-isolated-checklist-audit.md) / [ADR-087](../03-adr/ADR-087-identity-inventory-ratchet.md) / [ADR-088](../03-adr/ADR-088-mmio-high-alias.md) / [ADR-089](../03-adr/ADR-089-start-stub-identity-exception.md) / ADR-090 / ADR-091)
...
Version: **v1.66** (2026-09-26). Slice/update of v1.65 (#142 ADR-090). [ADR-091](../03-adr/ADR-091-el0-isolated-accept-draft.md) (M5, D4) is a **draft** umbrella sentence and scope, pending the sponsor's written accept. No posture change. The umbrella stays Planned / non-claim.

Prior v1.65 (2026-09-26). Slice/update of v1.64 (#141 ADR-089). [ADR-090](../03-adr/ADR-090-serror-evidence-class.md) (M4, sponsor D1): the umbrella's taken-SError evidence class is the B1 pinned-QEMU CI job (now unconditional) plus the B2-P one-off KVM run. Stock TCG still parks. Not the umbrella: the sponsor accept (B3) is still absent.

Prior v1.64 (2026-09-26). Slice/update of v1.63 (#140 ADR-088). [ADR-089](../03-adr/ADR-089-start-stub-identity-exception.md) (M3, sponsor D2): the `_start` stub page is the one documented identity exception, the checklist identity row is reworded accordingly and Met on this session's evidence. Docs only. Still **not** “EL0 isolated.”

Prior v1.63 (2026-09-26). Slice/update of v1.62 (#139 ADR-087). [ADR-088](../03-adr/ADR-088-mmio-high-alias.md) (M2, sponsor D3): GIC / PL011 / virtio-mmio are reached only through three EL1-only, XN TTBR1 Device blocks (6 MiB); the identity 1 GiB MMIO block is cleared from kernel and ASID-B TTBR0. The fail-closed inventory now allows only the `_start` stub page (`ident: inv k=1 u=1 a=1 leaks=0`). Still **not** “EL0 isolated.”

Prior v1.62 (2026-09-26). Slice/update of v1.61 (#138 ADR-086). [ADR-087](../03-adr/ADR-087-identity-inventory-ratchet.md) (M1): the RAM-side identity leftovers (low RAM below the image, the RO+X image padding tail, the pre-heap frame) are unmapped from kernel **and** user TTBR0. A committed fail-closed walk over kernel / user / ASID-B TTBR0 allows only the MMIO 1 GiB Device block and the `_start` stub page, and a planted identity page is caught (in-boot + `inv-leak-probe` kernel). The EL1-executable identity surface while EL0 runs shrinks from ≈ 1 MiB + 1 page to the single stub page. Still **not** “EL0 isolated.”

Prior v1.61 (2026-09-26). Slice/update of v1.60 (#137 ADR-085). [ADR-086](../03-adr/ADR-086-el0-isolated-checklist-audit.md): sponsor unlocked the umbrella item. The ADR-055/060 checklist audit found every §A mile and PAN enable Verified this session (CI-gated by `qemu-smoke.sh`). §B is **not** Met: taken SError is Verified only on opt-in paths (B1 pin, B2-P one-off), while the accepted smoke machine parks. A scratch TTBR0 walker shows five identity ranges left in kernel TTBR0 (MMIO 1 GiB Device block, low RAM below the image, `_start` stub page, image padding tail RO+X, `__kernel_end` page) and two in user TTBR0 (stub + tail, EL1-only). No umbrella accept exists. Still **not** “EL0 isolated.”

Prior v1.60 (2026-09-26). Slice/update of v1.59 (#136 ADR-084). [ADR-085](../03-adr/ADR-085-b2p-graviton-kvm-serror.md): sponsor **B2-P**. On an AWS Graviton3 `c7g.metal` (eu-north-1, Spot), QEMU `-accel kvm` with patch 0002 set `serror_pending` via `KVM_SET_VCPU_EVENTS`, and the opt-in `b2-serror` profile **took** the lower-EL SError while standing at EL0 (`el0: serror` / `b2: taken`; fail-closed smoke PASS). That makes B2 **Verified (opt-in, one-off paid run, not CI)**. Real hardware also exposed a latent `TCR_EL1.TG1` reserved-encoding bug that TCG masked; it is fixed (`TG1=4K`). Stock distro QEMU still parks. B1 pin unchanged. Still **not** “EL0 isolated”, not “runs on real hardware” beyond the b2 profile, not “secure.”

Prior v1.59 (2026-09-25): Slice/update of v1.58 (#130 ADR-083). [ADR-084](../03-adr/ADR-084-b2-free-runner-serror.md): sponsor **B2** spike (different runner/accelerator) on free GitHub-hosted runners is **blocked** — no arm64 KVM/HVF/WHPX; stock TCG still `machine does not provide NMIs`. B2 stays Planned / blocked on a sponsor decision. B1 opt-in pin (ADR-083) unchanged. Stock distro QEMU still parks. Still **not** “EL0 isolated” / BSD sockets / Linux/containers. Not a v2 model and not “secure.”

Prior v1.58 (2026-09-19): Slice/update of v1.57 (#129 ADR-082). [ADR-083](../03-adr/ADR-083-b1-qemu-nmi-pin.md) ships opt-in pinned QEMU v10.0.0 + `TYPE_NMI`→async SError; `CTOS_QEMU` + `CTOS_REQUIRE_TAKEN_SERROR=1` Verifies taken `el0: serror`. Stock distro QEMU still parks. Default CI/Docker stay on distro QEMU. Keep PAN enable + N1–N5 + samples. Still **not** “EL0 isolated” / BSD sockets / Linux/containers. Not a v2 model and not “secure.”
...
| PAN capability + enable | `ID_AA64MMFR1_EL1.PAN` printed; PSTATE.PAN enabled on cortex-a76; EL1-vs-EL0 fault. Historical a57 enable non-goal ([ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md)). | `src/pan.rs` ([ADR-026](../03-adr/ADR-026-pan-capability.md) / [ADR-079](../03-adr/ADR-079-pan-cpu-reopen.md) / [ADR-080](../03-adr/ADR-080-pan-enable-fault.md)) |
| Remaining isolation gaps | Shared boot-stub text stays by decision (`ident: start-stay`; never yank `_start`). PAN enable Verified on cortex-a76 ([ADR-080](../03-adr/ADR-080-pan-enable-fault.md)). Leftover identity RAM torn (`ident: ram*`, [ADR-049](../03-adr/ADR-049-identity-ram-tear.md)). Taken lower-EL SError **hard-stopped** on stock QEMU after ADR-081 re-probe (`el0: serror-park`; A-clear dormant prep; [ADR-053](../03-adr/ADR-053-taken-serror-hard-stop.md) / [ADR-081](../03-adr/ADR-081-taken-serror-reopen.md) / [ADR-082](../03-adr/ADR-082-b1-qemu-type-nmi.md)); Verified only under the B1 opt-in pin ([ADR-083](../03-adr/ADR-083-b1-qemu-nmi-pin.md)); B2 free-runner spike blocked ([ADR-084](../03-adr/ADR-084-b2-free-runner-serror.md)). | User TTBR0 + ASID + TTBR1 + torn live `.text` / `.rodata` / `.data`/stacks / heap / leftover frame RAM + EL0 entry without `VMALLE1` + lower-EL IRQ while standing + default IRQ-unmasked ERET + lower-EL FIQ while standing + SError park honesty + A-clear dormant prep + start-stay honesty + PAN enable + SVC ABI; umbrella isolation Planned/non-claim until checklist ([ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md) / [ADR-055](../03-adr/ADR-055-el0-isolated-checklist.md)); sponsor closure table [ADR-060](../03-adr/ADR-060-isolation-leftovers-closure-checklist.md) |
...
| virtio-net (N1 ARP + N2 ICMP + N3 UDP + N5 thin TCP + N3.x/N4/N5.x EL0 SVC) | Guest programs a virtio-mmio net DMA master for ARP, ICMP echo, one UDP datagram, then one thin TCP active-open + payload vs QEMU SLIRP/`guestfwd`. EL0 uses tiny SVCs only (kernel owns NIC), including quiet TCP echo. Not a BSD sockets / TCP product. | `src/virtio.rs` + `src/syscall.rs` ([ADR-066](../03-adr/ADR-066-virtio-net-first-frame.md), [ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md), [ADR-070](../03-adr/ADR-070-n3-udp-transport.md), [ADR-071](../03-adr/ADR-071-n3x-el0-udp-svc.md), [ADR-068](../03-adr/ADR-068-el0-net-svc-sample.md), [ADR-074](../03-adr/ADR-074-n5-thin-tcp.md), [ADR-076](../03-adr/ADR-076-el0-tcp-svc.md)) |
| virtio-blk + FAT16 | Guest programs a virtio-mmio DMA master. Image is host-built FAT16. Guest may write/create small root files ([ADR-050](../03-adr/ADR-050-fat16-write.md)), grow a file across a cluster boundary ([ADR-064](../03-adr/ADR-064-fat16-multi-cluster-grow.md)), list root names ([ADR-056](../03-adr/ADR-056-fat16-readdir.md)), delete a root file ([ADR-057](../03-adr/ADR-057-fat16-delete.md)), create a root directory ([ADR-073](../03-adr/ADR-073-fat16-mkdir.md)), and create one nested subdirectory + empty-rmdir ([ADR-077](../03-adr/ADR-077-fat16-nested-rmdir.md)). Same VFS `open`/`write`/`readdir`/`unlink`/`mkdir`/`rmdir` behind prefix mounts ([ADR-058](../03-adr/ADR-058-vfs-prefix-mounts.md)). Not a trusted disk. | `src/virtio.rs` + `src/fat.rs` + `src/vfs.rs` ([ADR-028](../03-adr/ADR-028-virtio-blk-fat16.md), [ADR-050](../03-adr/ADR-050-fat16-write.md), [ADR-056](../03-adr/ADR-056-fat16-readdir.md), [ADR-057](../03-adr/ADR-057-fat16-delete.md), [ADR-058](../03-adr/ADR-058-vfs-prefix-mounts.md), [ADR-064](../03-adr/ADR-064-fat16-multi-cluster-grow.md), [ADR-073](../03-adr/ADR-073-fat16-mkdir.md), [ADR-077](../03-adr/ADR-077-fat16-nested-rmdir.md)) |
...
### PAN enable + EL1-vs-EL0 fault (v1.55)

[ADR-080](../03-adr/ADR-080-pan-enable-fault.md) / [#125](https://github.com/artofdream/ctos/issues/125) mile 2: on default `-cpu cortex-a76`, `MSR PAN` + EL1 load of an EL0-accessible page faults (`pan: enabled` / `pan: el1-fault`; SPSR.PAN at the fault). Syscall uaccess clears PAN around copies. Umbrella “EL0 isolated” stays non-claim ([ADR-055](../03-adr/ADR-055-el0-isolated-checklist.md)). Taken SError still hard-stopped ([ADR-053](../03-adr/ADR-053-taken-serror-hard-stop.md)).

### Taken SError reopen re-probe (v1.56)

[ADR-081](../03-adr/ADR-081-taken-serror-reopen.md) / [#125](https://github.com/artofdream/ctos/issues/125) mile 3: dated agent-box re-probe on default `-cpu cortex-a76` plus GICv3 / `virtualization=on` / `secure=on` / `max` — QMP `inject-nmi` / HMP `nmi` still `machine does not provide NMIs`. FEAT_NMI ≠ SError. KVM absent on this smoke host. Unlock = sponsor B1/B2/B3. Park Verified; A-clear dormant prep. Still **not** taken Verified / “EL0 isolated.”

### Taken SError B2-P on real arm64 + KVM (v1.60)

[ADR-085](../03-adr/ADR-085-b2p-graviton-kvm-serror.md) (2026-09-26): a paid one-off run on AWS `c7g.metal` (Graviton3, `CPU: All CPU(s) started at EL2`, `/dev/kvm=present`). The SError source is **KVM's virtual SError** (`HCR_EL2.VSE`), which the host requests through `KVM_SET_VCPU_EVENTS{serror_pending=1}` (QEMU v10.0.0 + 0001 + 0002, `inject-nmi` on the `el0: serror-arm` cue). The guest runs natively. Run 4 console: `el0: standing` / `el0: serror` / `el0: restored` / `b2: taken` / `b2-kvm-smoke: PASS ... accel=kvm`. Threat-model relevance: the lower-EL SError vector, standing-EL0 A-clear window and restore path now have evidence on real Neoverse-V1 hardware, not only TCG. Real hardware also found `TCR_EL1.TG1=0b00` (reserved), which broke every TTBR1 high-VA walk on Graviton3; it is fixed to 4K. Limits: opt-in profile only (no GIC/timer/virtio/FAT under KVM), no CI gate, no host-side ioctl trace. Umbrella “EL0 isolated” still a non-claim.

### Taken SError B2 free-runner spike (v1.59)

[ADR-084](../03-adr/ADR-084-b2-free-runner-serror.md) (2026-09-25): sponsor chose ADR-081 **B2** — a genuinely different runner/accelerator, not the B1 pin. Free GitHub-hosted runners were probed (runs [36193423604](https://github.com/artofdream/ctos/actions/runs/36193423604) / [36193613972](https://github.com/artofdream/ctos/actions/runs/36193613972)): arm64 Linux `kvm [1]: HYP mode not available` / `/dev/kvm` absent; macOS arm64 HVF `HV_UNSUPPORTED`; Windows arm64 no VT / WHPX disabled; x86 `/dev/kvm` cannot run an AArch64 guest. Stock TCG on those runners still `machine does not provide NMIs`. The honest B2 source would be a KVM virtual SError (`serror_pending`) injected by a small VMM on real arm64 hardware — **Planned / blocked** on a sponsor choice (self-hosted arm64 KVM runner, paid bare metal, or wait). No new Verified. Threat surface unchanged; still **not** “EL0 isolated.”

### Residual identity surface + umbrella audit (v1.61)

[ADR-086](../03-adr/ADR-086-el0-isolated-checklist-audit.md) (2026-09-26). Threat-model relevance of the residual identity map: EL0 cannot touch any of it (every leaf is EL1-only, `el0=false`). The EL1-side exposure is: (1) an identity **RO+X** padding tail (≈ 1 MiB, no image content) and the stub page stay executable at EL1 through TTBR0, **including while EL0 runs** (user TTBR0 keeps both), which widens the EL1 gadget/ret2 surface through low VAs; (2) the MMIO block and two small RW RAM ranges stay reachable at EL1 through identity VAs, so a stray EL1 pointer into low VAs still hits memory instead of faulting. Mitigation plan: M1 ratchet + RAM-side tears, M2 MMIO high alias, M3 stub policy (sponsor D2). Taken SError evidence class is a sponsor decision (D1). Umbrella stays non-claim.

### Identity inventory ratchet (v1.62)

[ADR-087](../03-adr/ADR-087-identity-inventory-ratchet.md) (2026-09-26). Residual identity after M1: I1 MMIO block (EL1 RW, XN, Device; kernel + ASID-B only) and I3 `_start` stub page (EL1 RO+X; kernel + user + ASID-B). The ratchet turns “what we tore” into “what is still mapped”, so a regression that re-maps any identity page fails CI. Found while building it: the ADR-020/025 `.rodata` pointer rewrite silently relocated the allowlist constants themselves (compare physically). M2 removes I1; I3 stays as the one documented exception (sponsor D2).

### MMIO high alias (v1.63)

[ADR-088](../03-adr/ADR-088-mmio-high-alias.md) (2026-09-26). Threat-model effect: a stray EL1 pointer into low VAs `< 0x4000_0000` now faults instead of touching a device, and the device surface reachable at EL1 shrinks from a 1 GiB identity hole to three 2 MiB TTBR1 blocks. Residual identity: the `_start` stub page (EL1 RO+X) in kernel / user / ASID-B TTBR0, the documented exception (sponsor D2). B2-P KVM evidence predates this boot-path change.

### `_start` stub identity exception (v1.64)

[ADR-089](../03-adr/ADR-089-start-stub-identity-exception.md) (2026-09-26). The residual identity page is EL1 RO+X (never EL0-accessible) in kernel / user / ASID-B TTBR0; it holds `_start` and the identity copy of the vectors. Any additional identity leaf fails CI. A possible later narrowing (drop it from user TTBR0 only) is noted, not decided.

### Taken-SError evidence class (v1.65)

[ADR-090](../03-adr/ADR-090-serror-evidence-class.md) (2026-09-26). Threat-model effect: the lower-EL SError vector path (standing EL0, `PSTATE.A` clear) is exercised on every push/PR by a patched-QEMU TCG job; it is no longer opt-in. What this evidence does **not** show: RAS/hardware error semantics, SError on stock QEMU, or a CI-gated real-hardware run (B2-P was one paid run on a reduced profile at `159b178`).

### Umbrella accept draft: scope and non-scope (v1.66)

[ADR-091](../03-adr/ADR-091-el0-isolated-accept-draft.md) (2026-09-26, **Draft**). If accepted, the umbrella would cover only the architectural EL0→kernel page-table / exception-level boundary on the default QEMU TCG machine (one core). It explicitly excludes speculative execution / side channels (no KPTI: TTBR1 stays mapped EL1-only while EL0 runs), real hardware, multi-core, task-vs-task isolation beyond the ASID mile, DMA/IOMMU and EL2. Gaps recorded for the sponsor: no exhaustive EL0-permission walk (G1), no EL0 write probe (G2).
...
- Not secure boot, PAN enable on historical cortex-a57 (non-goal on that CPU, [ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md); enable Verified on cortex-a76 / [ADR-080](../03-adr/ADR-080-pan-enable-fault.md)), or a fully torn-down identity map (ADR-020/025/037/038/049 unmap live `.text` / `.rodata` / `.data`+stacks / heap / leftover frame RAM; `_start` stays at `0x4008_0000` by decision).
...
| SError park honesty | Serial `el0: serror-park` | No working SError inject on virt TCG ([ADR-043](../03-adr/ADR-043-lower-el-fiq-serror.md) / [ADR-045](../03-adr/ADR-045-taken-serror-qmp.md) / [ADR-081](../03-adr/ADR-081-taken-serror-reopen.md)). Taken path deferred/non-goal ([ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md)). |
| Taken SError QMP attempt | Serial `el0: serror-arm` + QMP log | Guest A-clear path ([ADR-045](../03-adr/ADR-045-taken-serror-qmp.md)); `inject-nmi` fails on QEMU 10 virt+a76 (ADR-081). |
...
| PAN ID field | Serial `pan: id=` / `pan: present` | Capability probe ([ADR-026](../03-adr/ADR-026-pan-capability.md) / [ADR-079](../03-adr/ADR-079-pan-cpu-reopen.md)). |
| PAN enable + EL1-vs-EL0 fault | Serial `pan: enabled` / `pan: el1-fault` | [ADR-080](../03-adr/ADR-080-pan-enable-fault.md). Not “EL0 isolated.” |
...
| virtio-blk + FAT16 | Serial `blk: ok` / `fat: ok` / `fat: write` / `fat: create` / `fat: grow` / `fat: mkdir` / `fat: nested` / `fat: rmdir` / `fat: readdir` / `fat: entries` / `fat: delete`; VFS `/probe` | block + FAT mile ([ADR-028](../03-adr/ADR-028-virtio-blk-fat16.md)) + write depth ([ADR-050](../03-adr/ADR-050-fat16-write.md)) + multi-cluster grow ([ADR-064](../03-adr/ADR-064-fat16-multi-cluster-grow.md)) + mkdir ([ADR-073](../03-adr/ADR-073-fat16-mkdir.md)) + nested+rmdir ([ADR-077](../03-adr/ADR-077-fat16-nested-rmdir.md)) + readdir ([ADR-056](../03-adr/ADR-056-fat16-readdir.md)) + delete ([ADR-057](../03-adr/ADR-057-fat16-delete.md)). Host `-drive` alone is not the probe. Not POSIX `getdents`/`unlink`/`mkdir`/`rmdir`/write API. |
...
[ADR-074](../03-adr/ADR-074-n5-thin-tcp.md): after N3 UDP, guest active-opens TCP to QEMU `guestfwd` `10.0.2.4:7`, sends one payload `ctos-tcp\n`, requires matching echo (`net: tcp-syn` / `net: tcp-est` / `net: tcp-tx` / `net: tcp-rx` / `net: tcp-ok`). QEMU `-netdev` gains `guestfwd=tcp:10.0.2.4:7-cmd:…/tcp-echo-stdio.sh`. Kernel-path N5. One connection; no listen/accept. [ADR-076](../03-adr/ADR-076-el0-tcp-svc.md) adds EL0 `net_tcp_echo` (28) + FAT `/tcpdemo` quiet re-run of the same probe. Still not BSD sockets / TCP product / TLS/HTTP / DHCP/DNS product / Wi‑Fi / “has networking / sockets OS.” Still **not** “secure.”
...
### PAN CPU reopen foundation (v1.54)

[ADR-079](../03-adr/ADR-079-pan-cpu-reopen.md) / [#125](https://github.com/artofdream/ctos/issues/125): sponsor unlock + default smoke CPU `-cpu cortex-a76` + Verified ID-field `pan: present` (ADR-054 gate items 1–2). Historical cortex-a57 `pan: absent` retained. Enable+fault is [ADR-080](../03-adr/ADR-080-pan-enable-fault.md) (v1.55). **Does not** unlock taken SError — confirmed blocked in [ADR-081](../03-adr/ADR-081-taken-serror-reopen.md). Umbrella “EL0 isolated” stays non-claim. Never yank `_start`. Still **not** “secure.”
