...
Version: **v1.30** (2026-09-14). Slice/update of v1.29. Isolation-gap locks: taken SError **hard-stopped** ([ADR-053](../03-adr/ADR-053-taken-serror-hard-stop.md)); PAN enable **locked** non-goal on default cortex-a57 ([ADR-054](../03-adr/ADR-054-pan-enable-lock.md)); umbrella “EL0 isolated” **non-claim until checklist** ([ADR-055](../03-adr/ADR-055-el0-isolated-checklist.md)). Product freestanding app-hosting claim remains **Verified** under ADR-048/052. Never yank `_start`; leftover identity RAM torn (`ident: ram*`). Still not Linux/POSIX/containers. Not a v2 model and not “secure.”
