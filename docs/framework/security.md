# Security — threat model v1.25 (NFR-10 / ADR-011 / ADR-012 / ADR-013 / ADR-014 / ADR-015 / ADR-016 / ADR-017 / ADR-018 / ADR-019 / ADR-020 / ADR-021 / ADR-022 / ADR-023 / ADR-024 / ADR-025 / ADR-026 / ADR-027 / ADR-028 / ADR-030 / ADR-032 / ADR-037 / ADR-038 / ADR-039 / ADR-040 / ADR-041 / ADR-042 / ADR-043 / ADR-044 / ADR-045)
...
Version: **v1.25** (2026-09-13). Slice/update of v1.24/v1.23 (adds ADR-045 A-clear standing SError probe + QMP `inject-nmi` attempt; inject fails on QEMU 10 virt+cortex-a57; PAN enable still Planned; taken SError still Planned; umbrella isolation still Planned). Not a v2 model and not “secure.”
...
| Remaining isolation gaps | Shared boot-stub text in the user table (`ident: start-stay`). No PAN enable on cortex-a57. Remaining identity RAM after the heap (`ident: ram-stay`). Taken lower-EL SError still Planned (`el0: serror-park`; ADR-045 QMP attempt failed). | User TTBR0 + ASID + TTBR1 + torn live `.text` / `.rodata` / `.data`/stacks / heap + EL0 entry without `VMALLE1` + lower-EL IRQ while standing + default IRQ-unmasked ERET + lower-EL FIQ while standing + SError park honesty + A-clear QMP attempt + start-stay honesty + SVC ABI; isolation Planned ([ADR-045](../03-adr/ADR-045-taken-serror-qmp.md)) |
...
| SError park honesty | Serial `el0: serror-park` | No working SError inject on this virt guest ([ADR-043](../03-adr/ADR-043-lower-el-fiq-serror.md) / [ADR-045](../03-adr/ADR-045-taken-serror-qmp.md)). Taken SError Planned. |
...
Research ([ADR-044](../03-adr/ADR-044-taken-serror-research.md)); implementation attempt ([ADR-045](../03-adr/ADR-045-taken-serror-qmp.md)): guest A-clear standing path + host QMP `inject-nmi`. On QEMU 10 virt+`-cpu cortex-a57`, QMP returns `machine does not provide NMIs` (no `ARM_CPU_SERROR` via nmi; FEAT_NMI needs a different CPU/GIC). Park marker remains Verified; taken path Planned. Not “EL0 isolated.”