# Security — threat model v1.10 (NFR-10 / ADR-011 / ADR-012 / ADR-013 / ADR-014 / ADR-015 / ADR-016 / ADR-017 / ADR-018 / ADR-019 / ADR-020 / ADR-021 / ADR-022)
...
Version: **v1.10** (2026-09-12). Slice/update of v1.9 (adds a freestanding `libctos` CRT wrapping `exit` / `uart_write` / `yield`; app hosting stays Planned; PAN and full identity teardown still Planned). Not a v2 model and not “secure.”
...
| `libctos` CRT | Wrappers + `_start` CRT. Hello payload copied onto the standing EL0 page. Must not issue SVC #0–#2. Not a guest ELF loader. | `libctos/` + `src/libctos.rs` ([ADR-022](../03-adr/ADR-022-libctos-crt.md)) |
...
| --- | --- | --- |
| Threat-model v1.10 written | Read this file | Verified (file + review). Still no “secure OS”. |
