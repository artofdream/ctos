...
Version: **v1.56** (2026-09-19). Slice/update of v1.55 (#127 ADR-080). Taken SError reopen re-probe ([ADR-081](../03-adr/ADR-081-taken-serror-reopen.md) / [#125](https://github.com/artofdream/ctos/issues/125) mile 3): still `machine does not provide NMIs` on virt TCG + cortex-a76 (and GICv3/`virtualization=on`/`max`); unlock = B1/B2/B3. Park Verified; A-clear dormant prep. Keep PAN enable + N1–N5 + samples. Still **not** “EL0 isolated” / BSD sockets / Linux/containers. Not a v2 model and not “secure.”
...
| Remaining isolation gaps | Shared boot-stub text stays by decision (`ident: start-stay`; never yank `_start`). PAN enable Verified on cortex-a76 ([ADR-080](../03-adr/ADR-080-pan-enable-fault.md)). Leftover identity RAM torn (`ident: ram*`, [ADR-049](../03-adr/ADR-049-identity-ram-tear.md)). Taken lower-EL SError **hard-stopped** after ADR-081 re-probe (`el0: serror-park`; A-clear dormant prep; [ADR-053](../03-adr/ADR-053-taken-serror-hard-stop.md) / [ADR-081](../03-adr/ADR-081-taken-serror-reopen.md)). | User TTBR0 + ASID + TTBR1 + torn live `.text` / `.rodata` / `.data`/stacks / heap / leftover frame RAM + EL0 entry without `VMALLE1` + lower-EL IRQ while standing + default IRQ-unmasked ERET + lower-EL FIQ while standing + SError park honesty + A-clear dormant prep + start-stay honesty + PAN enable + SVC ABI; umbrella isolation Planned/non-claim until checklist ([ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md) / [ADR-055](../03-adr/ADR-055-el0-isolated-checklist.md)); sponsor closure table [ADR-060](../03-adr/ADR-060-isolation-leftovers-closure-checklist.md) |
