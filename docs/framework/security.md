# Security — threat model v1.9 (NFR-10 / ADR-011 / ADR-012 / ADR-013 / ADR-014 / ADR-015 / ADR-016 / ADR-017 / ADR-018 / ADR-019 / ADR-020 / ADR-021)
...
Version: **v1.9** (2026-09-11). Slice/update of v1.8 (adds a documented EL0 SVC ABI — `exit` / `uart_write` / `yield`; app hosting stays Planned; PAN and full identity teardown still Planned). Not a v2 model and not “secure.”
...
| SVC ABI (exit / uart_write / yield) | Documented numbers 16–18. `uart_write` rejects a kernel `.data` pointer. Not Linux. Not app hosting. | `src/syscall.rs` ([ADR-021](../03-adr/ADR-021-svc-syscall-abi.md)) |
...
| --- | --- | --- |
| Threat-model v1.9 written | Read this file | Verified (file + review). Still no “secure OS”. |
