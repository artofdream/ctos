...
Version: **v1.50** (2026-09-17). Slice/update of v1.49 (#120 ADR-074). EL0 `fs_mkdir` ([ADR-075](../03-adr/ADR-075-el0-fs-mkdir.md): SVC 27 + FAT `/mkdemo`). Keep N1–N5 + ADR-069/072/073 + prior samples. Still **not** BSD sockets / TCP product / listen-accept / TLS/HTTP / DHCP/DNS product / Wi‑Fi / “has networking / sockets OS.” Still not Linux/POSIX/`mount`/`unlink`/`mkdir`/`getdents`/containers/preemption/exFAT. Not a v2 model and not “secure.”
