# Security — threat model v1.5 (NFR-10 / ADR-011 / ADR-012 / ADR-013 / ADR-014 / ADR-015 / ADR-016 / ADR-017)
...
Version: **v1.5** (2026-09-11). Slice/update of v1.4 (adds EL1 fetch from the TTBR1 RAM alias + high VBAR; PAN and identity teardown still Planned). Not a v2 model and not “secure.”
...
| TTBR1 RAM alias / high VBAR | EL1 can fetch `.text` at `identity + TTBR1_BASE`. `VBAR_EL1` is that alias. Identity `-kernel` stub stays. | `L1_HIGH[1]` → `L2_RAM` ([ADR-017](../03-adr/ADR-017-ttbr1-high-el1-exec.md)) |
| Remaining isolation gaps | Shared kernel text in the user table (handler must fetch if VBAR were still identity). No PAN on cortex-a57. EL0 trampoline still `TLBI VMALLE1` (`.data` leaves are global). Identity map still live. Lower-EL IRQ still parks. | User TTBR0 + ASID + TTBR1 first cut + exec mile; isolation Planned |
...
- **EL0 first mile + user TTBR0 + ASID TLB + standing + TTBR1 first cut + EL1 high-VA fetch, isolation Planned.** Lower-EL AArch64 **sync** is taken (SVC / IABORT / DABORT). IRQ/FIQ/SError lower-EL slots still park. Dual ASID without `TLBI VMALLE1` is a probed mile (`asid: ok`). Standing dual-SVC flips `is_active()`. TTBR1 private page is EL1-only (`ttbr1: ok`). EL1 can fetch a real path from the high RAM alias (`ttbr1: el1 exec`); `VBAR_EL1` is that alias (`ttbr1: vbar`). Closing *isolation* still needs PAN and a full identity teardown. A caught load of `.data` or of `TTBR1_PRIV` is that mile, not “EL0 isolated.”
...
- Not secure boot, PAN, or a torn-down identity map (ADR-017 is EL1 fetch from a high alias; `_start` stays at `0x4008_0000`).

## Mitigations mapped to probes

| Mitigation | Probe | Ledger |
| --- | --- | --- |
| Threat-model v1.5 written | Read this file | Verified (file + review). Still no “secure OS”. |
...
| EL1 fetch from TTBR1 high VA | Serial `ttbr1: el1 exec` / `ttbr1: vbar` | Exec mile ([ADR-017](../03-adr/ADR-017-ttbr1-high-el1-exec.md)). Identity teardown Planned. |
| RO+NX text/data | Serial `ro: ok`; execute-from-`.data` + write-to-RO-text | Verified: 2026-09-11 cloud `qemu-smoke` (honesty ledger). |

## Claim gate

A PR may say “threat-model v1.5 exists” after a file read. It may say “heap NX” / “identity image is W^X on this virt guest” only when the honesty ledger has a matching **Verified** QEMU probe. It may say “linker-stack guard faults” only with a matching translation-abort probe. It may say “EL0 entered and returned,” “EL0 cannot execute kernel data,” “EL0 cannot read kernel `.data`,” “standing EL0 context,” “ASID isolation,” “TTBR1 private page,” or “EL1 fetched from a TTBR1 high VA” only for the mile that actually passed.
