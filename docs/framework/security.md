# Security — threat model v1.6 (NFR-10 / ADR-011 / ADR-012 / ADR-013 / ADR-014 / ADR-015 / ADR-016 / ADR-017 / ADR-018)
...
Version: **v1.6** (2026-09-11). Slice/update of v1.5 (adds split TTBR1 RAM tables + one torn identity text page; PAN and full identity teardown still Planned). Not a v2 model and not “secure.”
...
| Remaining isolation gaps | Shared kernel text in the user table (handler must fetch if VBAR were still identity). No PAN on cortex-a57. EL0 trampoline still `TLBI VMALLE1` (`.data` leaves are global). Most of the identity map still live. Lower-EL IRQ still parks. | User TTBR0 + ASID + TTBR1 first cut + exec mile + one torn page; isolation Planned |
...
- **EL0 first mile + user TTBR0 + ASID TLB + standing + TTBR1 first cut + EL1 high-VA fetch + identity-tear first cut, isolation Planned.** Lower-EL AArch64 **sync** is taken (SVC / IABORT / DABORT). IRQ/FIQ/SError lower-EL slots still park. Dual ASID without `TLBI VMALLE1` is a probed mile (`asid: ok`). Standing dual-SVC flips `is_active()`. TTBR1 private page is EL1-only (`ttbr1: ok`). EL1 can fetch a real path from the high RAM alias (`ttbr1: el1 exec`); `VBAR_EL1` is that alias (`ttbr1: vbar`). One identity text page is unmapped (`ident: ok`); `_start` stays. Closing *isolation* still needs PAN and a full identity teardown. A caught load of `.data` or of `TTBR1_PRIV` is that mile, not “EL0 isolated.”
...
- Not secure boot, PAN, or a fully torn-down identity map (ADR-018 unmaps one text page; `_start` stays at `0x4008_0000`).

## Mitigations mapped to probes

| Mitigation | Probe | Ledger |
| --- | --- | --- |
| Threat-model v1.6 written | Read this file | Verified (file + review). Still no “secure OS”. |
...
A PR may say “threat-model v1.6 exists” after a file read. It may say “heap NX” / “identity image is W^X on this virt guest” only when the honesty ledger has a matching **Verified** QEMU probe. It may say “linker-stack guard faults” only with a matching translation-abort probe. It may say “EL0 entered and returned,” “EL0 cannot execute kernel data,” “EL0 cannot read kernel `.data`,” “standing EL0 context,” “ASID isolation,” “TTBR1 private page,” “EL1 fetched from a TTBR1 high VA,” or “one identity text page was unmapped” only for the mile that actually passed.
