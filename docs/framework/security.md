...
Version: **v1.42** (2026-09-16). Slice/update of v1.41 (#112 ADR-066). Track N / N2 ICMP ping after ARP ([ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md): `net: icmp-tx` / `net: icmp-rx` / `net: ping-ok`; keep N1 `net: ok`). Still **not** TCP/UDP/sockets/DHCP/DNS/Wi‑Fi/“has networking.” ADR-066 N1 unchanged. ADR-063 N0 scope unchanged. ADR-062/061/060/053/054/055 locks unchanged. Product freestanding app-hosting claim remains **Verified** under ADR-048/052. Still not Linux/POSIX/`mount`/`unlink`/`getdents`/containers/preemption/exFAT. Not a v2 model and not “secure.”
...
**Out:** Raspberry Pi or other boards, a second ISA, multi-tenant hosting, secure / measured boot, physical side channels, a hostile hypervisor. **Networking:** N1 ARP first-frame + N2 ICMP ping ([ADR-066](../03-adr/ADR-066-virtio-net-first-frame.md) / [ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md)); not a TCP/UDP/sockets product stack.
...
| virtio-net (N1 ARP + N2 ICMP) | Guest programs a virtio-mmio net DMA master for ARP TX/RX then ICMP echo vs QEMU SLIRP. Not a TCP/UDP stack. | `src/virtio.rs` ([ADR-066](../03-adr/ADR-066-virtio-net-first-frame.md), [ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md)) |
...
| **DMA / virtio devices** | **Named; bounded** | A7 programs virtio-mmio blk (one queue, poll `used.idx`, identity-PA DMA). N1–N2 program virtio-mmio net (RX+TX queues, same poll/DMA pattern) for ARP then ICMP echo ([ADR-066](../03-adr/ADR-066-virtio-net-first-frame.md) / [ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md)). QEMU is still TCB. A malicious virtio device is out (hostile hypervisor). No TCP/UDP stack. |
...
| **Network attacker** | **Named; bounded (link only)** | N1–N2 attach QEMU user netdev + guest virtio-net for ARP + ICMP echo ([ADR-066](../03-adr/ADR-066-virtio-net-first-frame.md) / [ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md)). No TCP/UDP/sockets. QEMU SLIRP is still TCB. Hostile on-wire peers beyond this probe are out of product scope. |
...
| virtio-net ARP (N1) + ICMP ping (N2) | Serial `net: virtio` / `net: mac` / `net: tx` / `net: rx` / `net: ok` + `net: icmp-tx` / `net: icmp-rx` / `net: ping-ok` | [ADR-066](../03-adr/ADR-066-virtio-net-first-frame.md) / [ADR-067](../03-adr/ADR-067-virtio-net-icmp-ping.md). ARP + ICMP vs QEMU SLIRP only. Not TCP/UDP/sockets. Host `-netdev` alone is not the probe. |
