# Security — threat model v1.4 (NFR-10 / ADR-011 / ADR-012 / ADR-013 / ADR-014 / ADR-015 / ADR-016)
...
Version: **v1.4** (2026-09-11). Slice/update of v1.3 (adds standing EL0 + TTBR1 private-page first cut; PAN and identity teardown still Planned). Not a v2 model and not “secure.”

## Scope

**In:** the `ctos` guest as built for `qemu-system-aarch64 -machine virt` (EL1, identity map + TTBR1 private page, PL011, GICv2, CNTP, first-fit heap, cooperative EL1 tasks, a deliberate EL0 first mile and a bounded standing context). The git repo (no secrets).
...
| Remaining isolation gaps | Shared kernel text in the user table (handler must fetch). No PAN on cortex-a57. EL0 trampoline still `TLBI VMALLE1` (`.data` leaves are global). Identity map still live. Lower-EL IRQ still parks. | User TTBR0 + ASID + TTBR1 first cut; isolation Planned |
...
- **EL0 first mile + user TTBR0 + ASID TLB + standing + TTBR1 first cut, isolation Planned.** Lower-EL AArch64 **sync** is taken (SVC / IABORT / DABORT). IRQ/FIQ/SError lower-EL slots still park. Dual ASID without `TLBI VMALLE1` is a probed mile (`asid: ok`). Standing dual-SVC flips `is_active()`. TTBR1 private page is EL1-only (`ttbr1: ok`). Closing *isolation* still needs PAN and a full higher-half / identity teardown. A caught load of `.data` or of `TTBR1_PRIV` is that mile, not “EL0 isolated.”
...
- Not secure boot, PAN, or a relocated higher-half kernel (TTBR1 first cut is one private page).

## Mitigations mapped to probes

| Mitigation | Probe | Ledger |
| --- | --- | --- |
| Threat-model v1.4 written | Read this file | Verified (file + review). Still no “secure OS”. |
