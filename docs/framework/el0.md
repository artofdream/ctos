...
- Public SVC ABI ([syscall.md](syscall.md)): `exit` / `uart_write` / `yield`. A `libctos` CRT wraps those numbers; a hello payload is copied onto the standing EL0 page (A2) and also parsed as ELF64 `PT_LOAD` on the guest (A3). ABI + CRT + loader miles. App hosting Planned.
...
| App hosting (Track A A4–A9) | Standing EL0 as normal mode, VFS, sample apps ([issue #31](https://github.com/artofdream/ctos/issues/31)). The ABI + CRT + loader miles are not that. |
