...
*Principles sit above pillars. Tracks sit below both. A1 is a kernel ABI mile only. Not a claim that Track A hosting is built.*
...
3. **A short lower-privilege stub** — a few instructions in the CPU’s user mode, including the A1 SVC ABI trip (`exit` / `uart_write` / `yield`), then a call back into the kernel. **Not a process.** No libc, no files, no apps.
