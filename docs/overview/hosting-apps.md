...
| **ELF / user loader** | A3: guest ELF64 `PT_LOAD` into user TTBR0, then `ERET` to `e_entry` ([ADR-023](../03-adr/ADR-023-elf-pt-load-loader.md)). A2–A4 still embed. A9 also loads FAT `/hello`. Not a Linux ELF ABI. | **Loader mile** — Verified only when the ledger has `loader: ok` on this tip. Not app hosting. |
| **OS vs app slots** | A9: host kernel ELF + `hello-libctos.elf`; guest FAT `/hello` ([ADR-030](../03-adr/ADR-030-os-app-slots.md)). Same ELF on this OS and `ba6541c` ([ADR-031](../03-adr/ADR-031-track-a-leftovers.md)). | **Slot first cut + leftover** — Verified only when the ledger has `slot: ok` and the cross-update host lines on this tip. Not “apps update independently.” |
...
“Host an application independently of the OS” is still a sentence we do not use (product app hosting stays Planned). That block is **Track A**. A1 landed the kernel SVC ABI. A2 landed `libctos`. A3 landed a guest `PT_LOAD` loader. A4 landed standing EL0 as the supported path for that loaded image. A5 landed the identity `.rodata` tear and a PAN ID-field probe (enable Planned). A6 landed thin VFS + memfs. A7 landed virtio-blk + FAT16. A8 documented rebuild recipes ([what-can-run.md](what-can-run.md)). A9 landed the **slot first cut** (two host artifacts + FAT `/hello`). Leftover mile: A2–A4 load FAT `/hello` (no embed); same published ELF on this OS and `ba6541c`. See [Immutability](advantages.md#immutability).
