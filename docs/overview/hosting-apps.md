...
| **OS vs app slots** | A9: host kernel ELF + `hello-libctos.elf`; guest FAT `/hello` ([ADR-030](../03-adr/ADR-030-os-app-slots.md)). Cross-update Planned. | **Slot first cut** — Verified only when the ledger has `slot: ok` on this tip. Not “apps update independently.” |
...
Until a cross-update probe exists, “host an application independently of the OS” is a sentence we do not use. That block is **Track A**. A1 landed the kernel SVC ABI. A2 landed `libctos`. A3 landed a guest `PT_LOAD` loader (embedded image, not a filesystem `exec`). A4 landed standing EL0 as the supported path for that loaded image. A5 landed the identity `.rodata` tear and a PAN ID-field probe (enable Planned). A6 landed thin VFS + memfs. A7 landed virtio-blk + FAT16. A8 documented rebuild recipes ([what-can-run.md](what-can-run.md)). A9 landed the **slot first cut** (two host artifacts + FAT `/hello`; A2–A4 still embed). See [Immutability](advantages.md#immutability).
