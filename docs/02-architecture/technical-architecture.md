...
- `cargo test` uses `#![feature(custom_test_frameworks)]` and `#[test_case]` (including VBAR, BRK, SPSel, stack ranges, guards, GIC TYPER, CNTFRQ, timer tick, IRQ-delta samples, empty UART RX FIFO, MMU on, frames, map/unmap, heap `Box`/`Vec`, two-task yield, CNTPCT loop, W^X flags + execute-from-heap, EL0 first mile + standing enter/leave, SVC ABI yield/uart/exit + kernel-`.data` reject, ASID isolation, TTBR1 private page + high-VA EL1 fetch, identity-tear first cut + `.text` range tear + vtable rewrite + live `.text` tear; `is_active()` is false at rest)
