...
QEMU still `-kernel`s one **OS** ELF. A separate published app ELF lives on FAT **`/hello`**. A2–A4 load that file (no production embed). Cross-update is the same ELF on this OS and prior OS `ba6541c`. Product freestanding “app hosting done” is **Verified** under [ADR-048](../03-adr/ADR-048-app-hosting-claim-criteria.md) / [ADR-052](../03-adr/ADR-052-sponsor-accept-app-hosting.md) — not Linux/POSIX/containers/OTA. Companion: [hosting-apps.md](../overview/hosting-apps.md).

```mermaid
flowchart LR
  OS["OS ELF<br/>QEMU -kernel"] --> FAT["FAT16 volume<br/>virtio-blk"]
  APP["Published app ELF<br/>hello-libctos.elf"] --> FAT
  FAT -->|"/hello"| LOAD["Guest PT_LOAD<br/>→ standing EL0"]
```

*Two host artifacts, one guest volume. Slots disconnect OS rebuilds from the app ELF. Not a container runtime.*
