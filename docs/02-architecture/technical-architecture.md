...
- Host smoke: `scripts/qemu-smoke.sh` (hello + paging + heap + two-task sched + W^X + guards + EL0 first mile + standing + ASID isolation + TTBR1 private page + high-VA EL1 fetch + identity-tear first cut + identity `.text` range tear + vtable rewrite + live `.text` tear + CNTPCT baseline + IRQ-delta + host ELF size + timer tick + injected UART RX + BRK + fatal nested strings + tests + `force-fail` must be non-zero)
...
| Host apps / containers | Gaps to Linux/shell/Python: [host-apps.md](../framework/host-apps.md). Guest container runtime is a **non-goal**. Host `docker-smoke` is unrelated. |
