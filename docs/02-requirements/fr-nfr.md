...
**NFR-10 / NFR-08 note (2026-09-11):** [ADR-015](../03-adr/ADR-015-ro-nx-text-data.md) revises NFR-10 text in place for the RO+NX text/data cut (`SCTLR.WXN` on). ADR-013 adds a user-TTBR0 read mile, an ASID-tagged TLB mile, and a standing EL0 context; [ADR-016](../03-adr/ADR-016-ttbr1-private-page.md) adds a TTBR1 private-page first cut; [ADR-017](../03-adr/ADR-017-ttbr1-high-el1-exec.md) adds EL1 fetch from the TTBR1 RAM alias. The umbrella isolation row stays Planned (PAN + identity teardown still missing). NFR-08 boot-to-ready CNTPCT is a measurement, not a budget. IDs unchanged.
...
- EL1 fetch from the TTBR1 RAM alias (`ttbr1: el1 exec` / `ttbr1: vbar`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-017) **or** the honesty ledger says Unknown / Planned until probed. Identity `-kernel` stub at `0x40080000` stays. Full identity teardown stays Planned. Umbrella isolation stays Planned. Do not claim PAN.
