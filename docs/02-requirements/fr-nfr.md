...
- Identity-tear first cut (`ident: split` / `ident: fault` / `ident: high` / `ident: no el0` / `ident: ok`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-018) **or** the honesty ledger says Unknown / Planned until probed. Full identity teardown stays Planned. Umbrella isolation stays Planned. Do not claim PAN.
