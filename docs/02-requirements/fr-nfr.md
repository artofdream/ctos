...
- Thin VFS + memfs (`fs: create` / `fs: write` / `fs: read` / `fs: el0` / `fs: ok`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-027 / Track A A6) **or** the honesty ledger says Unknown / Planned until probed. Not POSIX. App hosting stays Planned.
- virtio-blk + FAT16 (`blk: virtio` / `blk: cap` / `blk: rw` / `blk: ok` / `fat: mount` / `fat: read` / `fat: ok`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-028 / Track A A7) **or** the honesty ledger says Unknown / Planned until probed. Same VFS `open`. Not POSIX. Not FAT32. App hosting stays Planned.
- FAT16 write depth (`fat: write` / `fat: rewrite` / `fat: create` / host `--check-write`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-050) **or** the honesty ledger says Unknown / Planned until probed. Same VFS `write`. Not POSIX. Not FAT32. App hosting stays Planned.
