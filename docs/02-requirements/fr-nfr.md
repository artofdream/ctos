...
- `libctos` CRT (`libctos: hi` / `libctos: ok` / `libctos: linked`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-022 / Track A A2) **or** the honesty ledger says Unknown / Planned until probed. Not a guest ELF loader. App hosting stays Planned.
