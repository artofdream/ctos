...
**NFR-10 note (2026-09-17, ADR-078):** EL0 `net_tcp_echo` quiet-path CNTPCT (`perf: tcp-echo ticks=<n>`, [ADR-078](../03-adr/ADR-078-tcp-echo-cntpct.md)). Threat-model **v1.53**. Measure-first only — not a latency SLA. Not BSD sockets. Not listen/accept. Keep `net: tcp-ok` / `libctos: tcp-ok`. IDs unchanged.
...
**NFR-10 note (2026-09-12):** [ADR-039](../03-adr/ADR-039-el0-entry-without-vmalle1.md) revises NFR-10 text in place for the EL0 trampoline without `TLBI VMALLE1` cut (after ADR-037/038). Threat-model v1.19. IDs unchanged.

**NFR-10 note (2026-09-13):** [ADR-040](../03-adr/ADR-040-lower-el-irq-standing.md) revises NFR-10 text in place for taken lower-EL IRQ while standing. Threat-model v1.20. FIQ/SError lower-EL still park. IDs unchanged.

**NFR-10 note (2026-09-13, ADR-041):** [ADR-041](../03-adr/ADR-041-irq-unmasked-default-eret.md) revises NFR-10 text in place for IRQ-unmasked default standing/task `ERET`. Threat-model v1.21. Short non-standing trampoline probes stay masked. FIQ/SError lower-EL still park. IDs unchanged.

**NFR-10 note (2026-09-13, ADR-049):** [ADR-049](../03-adr/ADR-049-identity-ram-tear.md) revises NFR-10 text in place for leftover identity RAM tear (`ident: ram` / `ident: ram-fault` / `ident: ram-high`; `ident: ram-stay` superseded). Threat-model v1.27. Umbrella isolation / PAN enable / taken SError / yank `_start` stay decided as in ADR-047. IDs unchanged.

**NFR-10 note (2026-09-13, ADR-042):** [ADR-042](../03-adr/ADR-042-isolation-leftover-wrap.md) revises NFR-10 text in place for the isolation leftover honesty wrap (`ident: start-stay` / `ident: ram-stay`). Threat-model v1.22. Umbrella isolation / PAN enable / FIQ-SError / full `_start` teardown stay Planned. IDs unchanged.

**NFR-10 note (2026-09-13, ADR-043):** [ADR-043](../03-adr/ADR-043-lower-el-fiq-serror.md) revises NFR-10 text in place for taken lower-EL FIQ while standing (`el0: fiq`) and SError park honesty (`el0: serror-park`). Threat-model v1.23. Taken SError / PAN enable / umbrella isolation stay Planned. IDs unchanged.

**NFR-10 note (2026-09-13, ADR-047 / ADR-048):** [ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md) decides isolation leftovers (taken SError deferred/non-goal on this smoke machine; PAN enable non-goal on default cortex-a57; never yank `_start`; leftover RAM optional Planned; umbrella Planned/non-claim). [ADR-048](../03-adr/ADR-048-app-hosting-claim-criteria.md) writes product app-hosting claim criteria (still Planned at that note). Threat-model **v1.26**. IDs unchanged.

**NFR-10 note (2026-09-13, ADR-048 / ADR-052):** Product app-hosting claim is **Verified** under [ADR-048](../03-adr/ADR-048-app-hosting-claim-criteria.md) criteria Met×5; sponsor accept recorded in [ADR-052](../03-adr/ADR-052-sponsor-accept-app-hosting.md). Threat-model **v1.29**. Still not Linux/POSIX/containers/“EL0 isolated.” IDs unchanged.

**NFR-10 note (2026-09-14, ADR-053 / ADR-054 / ADR-055):** Isolation-gap locks — taken SError hard-stopped ([ADR-053](../03-adr/ADR-053-taken-serror-hard-stop.md)); PAN enable locked non-goal on default cortex-a57 ([ADR-054](../03-adr/ADR-054-pan-enable-lock.md)); umbrella “EL0 isolated” non-claim until checklist ([ADR-055](../03-adr/ADR-055-el0-isolated-checklist.md)). Threat-model **v1.30**. IDs unchanged.

**NFR-10 note (2026-09-14, ADR-056):** FAT16 root listing behind the thin VFS (`fat: readdir` / `fat: entries`, [ADR-056](../03-adr/ADR-056-fat16-readdir.md)). Threat-model **v1.31**. Not POSIX `getdents`. IDs unchanged.

**NFR-10 note (2026-09-14, ADR-057):** FAT16 root delete behind the thin VFS (`fat: delete`, [ADR-057](../03-adr/ADR-057-fat16-delete.md)). Threat-model **v1.32**. Not POSIX `unlink`. IDs unchanged.

**NFR-10 note (2026-09-14, ADR-058):** Thin VFS prefix mounts (`vfs: mount` / `vfs: mounts`, [ADR-058](../03-adr/ADR-058-vfs-prefix-mounts.md)). Threat-model **v1.33**. Not POSIX `mount(2)`. IDs unchanged.
...
| **NFR-10 Security** | First-class pillar ([ADR-011](../03-adr/ADR-011-three-pillars.md)). No secrets in repo. Do not claim “secure OS” / “hardened” / “the kernel is W^X” without a written threat model **and** a probe. Prefer minimize `unsafe` (NFR-01). Heap and heap-backed cooperative stacks are PXN ([ADR-012](../03-adr/ADR-012-wx-nx-heap-stacks.md)). Linker-stack downward overflow hits an unmapped 4 KiB guard ([ADR-014](../03-adr/ADR-014-linker-stack-guard-pages.md)). `.text`/`.rodata` are RO+X and `.data`/`.bss`/live linker stacks are RW+NX; `SCTLR.WXN` is on ([ADR-015](../03-adr/ADR-015-ro-nx-text-data.md)). That scoped image cut is not a “secure OS” sentence. Device MMIO XN. Least privilege on IRQ paths. EL0 first mile (enter/return + cannot execute kernel data), the user-TTBR0 read mile, the ASID-tagged TLB mile, and the standing EL0 context are probes ([ADR-013](../03-adr/ADR-013-el0-isolation-direction.md)). TTBR1 private-page first cut is a probe ([ADR-016](../03-adr/ADR-016-ttbr1-private-page.md)). EL1 fetch from the TTBR1 RAM alias (plus high `VBAR_EL1`) is a probe ([ADR-017](../03-adr/ADR-017-ttbr1-high-el1-exec.md)). The first identity-tear cut (split TTBR1 RAM tables + one unmapped identity text page) is a probe ([ADR-018](../03-adr/ADR-018-identity-teardown-first-cut.md)). A 16 KiB dedicated identity text range plus a high-VA continuation is a probe ([ADR-019](../03-adr/ADR-019-identity-text-range-tear.md)). A high-VA vtable / fn-pointer rewrite plus live identity `.text` tear after the boot stub is a probe ([ADR-020](../03-adr/ADR-020-identity-fnptr-reloc.md)); identity `-kernel` stub stays. An identity `.rodata` tear after a pointer rewrite is a probe ([ADR-025](../03-adr/ADR-025-identity-rodata-tear.md)); `.data`/heap stay; full identity teardown stays Planned. A PAN ID-field read on `-cpu cortex-a57` is a probe ([ADR-026](../03-adr/ADR-026-pan-capability.md)); PAN enable stays Planned. A documented EL0 SVC ABI (`exit` / `uart_write` / `yield`) is a probe ([ADR-021](../03-adr/ADR-021-svc-syscall-abi.md)). A freestanding `libctos` CRT wrapping those numbers is a probe ([ADR-022](../03-adr/ADR-022-libctos-crt.md)). A guest ELF64 `PT_LOAD` loader into user TTBR0 is a probe ([ADR-023](../03-adr/ADR-023-elf-pt-load-loader.md)); not a Linux ABI. Standing EL0 as normal mode for a loaded image until `exit` (fail-closed restore) is a probe ([ADR-024](../03-adr/ADR-024-standing-el0-normal.md)). A thin VFS + in-RAM memfs (create / open / read / write / close; SVC 19–23) is a probe ([ADR-027](../03-adr/ADR-027-thin-vfs-memfs.md)); not POSIX (product freestanding hosting is Verified under ADR-048/052 separately). virtio-mmio block + FAT16 behind that same VFS (`/probe` read) is a probe ([ADR-028](../03-adr/ADR-028-virtio-blk-fat16.md)); FAT16 write + small create is a probe ([ADR-050](../03-adr/ADR-050-fat16-write.md)); FAT16 root listing is a probe ([ADR-056](../03-adr/ADR-056-fat16-readdir.md)); FAT16 root delete is a probe ([ADR-057](../03-adr/ADR-057-fat16-delete.md)); thin VFS prefix mounts are a probe ([ADR-058](../03-adr/ADR-058-vfs-prefix-mounts.md)); host `-drive` without guest code is not that probe. An OS-image vs app-payload slot first cut (published `hello-libctos.elf` + FAT `/hello` + A3 map) is a probe ([ADR-030](../03-adr/ADR-030-os-app-slots.md)). A2–A4 load that FAT file (no hello `include_bytes!`); the same published ELF on this OS and prior `ba6541c` is a probe ([ADR-032](../03-adr/ADR-032-track-a-leftovers.md)). Identity `.data`/stacks and heap are torn (`ident: data` / `ident: heap`, [ADR-037](../03-adr/ADR-037-identity-data-tear.md) / [ADR-038](../03-adr/ADR-038-identity-heap-tear.md)); standing/EL0 trampoline switches without `TLBI VMALLE1` (`el0: no-vmalle1`, [ADR-039](../03-adr/ADR-039-el0-entry-without-vmalle1.md)); taken lower-EL IRQ while standing returns to EL0 (`el0: irq`, [ADR-040](../03-adr/ADR-040-lower-el-irq-standing.md)); default standing/task `ERET` clears IRQ mask (`el0: irq-default`, [ADR-041](../03-adr/ADR-041-irq-unmasked-default-eret.md)); taken lower-EL FIQ while standing (`el0: fiq`, [ADR-043](../03-adr/ADR-043-lower-el-fiq-serror.md)); SError park honesty (`el0: serror-park`, [ADR-045](../03-adr/ADR-045-taken-serror-qmp.md) A-clear + QMP attempt); short non-standing trampoline probes stay masked; `_start` stays (`ident: start-stay`); leftover identity frames after the heap are torn (`ident: ram*`, [ADR-049](../03-adr/ADR-049-identity-ram-tear.md)); taken SError is hard-stopped/deferred ([ADR-053](../03-adr/ADR-053-taken-serror-hard-stop.md)); PAN enable is locked non-goal on default cortex-a57 ([ADR-054](../03-adr/ADR-054-pan-enable-lock.md)); never yank `_start`; umbrella isolation stays Planned/non-claim until checklist ([ADR-055](../03-adr/ADR-055-el0-isolated-checklist.md) / [ADR-047](../03-adr/ADR-047-isolation-leftovers-decisions.md)); product “app hosting is done” is **Verified** under [ADR-048](../03-adr/ADR-048-app-hosting-claim-criteria.md) / [ADR-052](../03-adr/ADR-052-sponsor-accept-app-hosting.md) (freestanding slots; not Linux/POSIX/containers). Do not claim PAN on `-cpu cortex-a57`. | Must | Now |
...
- Identity-tear first cut (`ident: split` / `ident: fault` / `ident: high` / `ident: no el0` / `ident: ok`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-018) **or** the honesty ledger says Unknown / Planned until probed. High-VA continuation plus a 16 KiB dedicated identity text range (`ident: jump` / `ident: range` / `ident: text`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-019) **or** the honesty ledger says Unknown / Planned until probed. High-VA vtable rewrite plus live identity `.text` tear (`ident: reloc` / `ident: live`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-020) **or** the honesty ledger says Unknown / Planned until probed. Identity `.rodata` tear (`ident: rodata` / `ident: rodata-fault` / `ident: rodata-high`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-025) **or** the honesty ledger says Unknown / Planned until probed. PAN ID field (`pan: id=` / `pan: absent`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-026) **or** the honesty ledger says Unknown / Planned until probed. Identity `.data`/stacks tear (`ident: data` / `ident: data-fault` / `ident: data-high`) is observable (NFR-10 / ADR-037) **or** the honesty ledger says Unknown / Planned until probed. Identity heap tear (`ident: heap-reloc` / `ident: heap` / `ident: heap-fault` / `ident: heap-high`) is observable (NFR-10 / ADR-038) **or** the honesty ledger says Unknown / Planned until probed. EL0 entry without `TLBI VMALLE1` (`el0: no-vmalle1`) is observable (NFR-10 / ADR-039) **or** the honesty ledger says Unknown / Planned until probed. Lower-EL IRQ while standing (`el0: irq`) is observable (NFR-10 / ADR-040) **or** the honesty ledger says Unknown / Planned until probed. Lower-EL FIQ while standing (`el0: fiq`) and SError park honesty (`el0: serror-park`) are observable (NFR-10 / ADR-043 / ADR-045) **or** the honesty ledger says Unknown / Planned until probed. Yank `_start` decided never; leftover identity RAM after the heap is torn ([ADR-049](../03-adr/ADR-049-identity-ram-tear.md)). Taken lower-EL SError is deferred/non-goal on this smoke machine. PAN enable is non-goal on default cortex-a57. Umbrella isolation stays Planned/non-claim. Do not claim PAN.
- EL0 SVC ABI (`svc: yield` / `svc: user-hi` / `svc: uart` / `svc: exit` / `svc: ok`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-021 / Track A A1) **or** the honesty ledger says Unknown / Planned until probed. This mile alone ≠ the ADR-048 product claim (Verified separately under ADR-048/052). Not a Linux ABI.
- `libctos` CRT (`libctos: hi` / `libctos: ok` / `libctos: linked`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-022 / Track A A2) **or** the honesty ledger says Unknown / Planned until probed. This mile alone ≠ the ADR-048 product claim (Verified separately under ADR-048/052).
- Guest ELF PT_LOAD loader (`loader: mapped` / `loader: ok`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-023 / Track A A3) **or** the honesty ledger says Unknown / Planned until probed. Not a Linux ABI. This mile alone ≠ the ADR-048 product claim (Verified separately under ADR-048/052).
- Standing EL0 as normal mode (`el0: task-enter` / `el0: task-active` / `el0: task-exit` / `el0: task-restored` / `el0: restore-fail` / `el0: task-ok`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-024 / Track A A4) **or** the honesty ledger says Unknown / Planned until probed. Not isolation. This mile alone ≠ the ADR-048 product claim (Verified separately under ADR-048/052).
- Thin VFS + memfs (`fs: create` / `fs: write` / `fs: read` / `fs: el0` / `fs: ok`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-027 / Track A A6) **or** the honesty ledger says Unknown / Planned until probed. Not POSIX. This mile alone ≠ the ADR-048 product claim (Verified separately under ADR-048/052).
- virtio-blk + FAT16 (`blk: virtio` / `blk: cap` / `blk: rw` / `blk: ok` / `fat: mount` / `fat: read` / `fat: ok`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-028 / Track A A7) **or** the honesty ledger says Unknown / Planned until probed. Same VFS `open`. Not POSIX. Not FAT32. This mile alone ≠ the ADR-048 product claim (Verified separately under ADR-048/052).
- FAT16 write depth (`fat: write` / `fat: rewrite` / `fat: create` / host `--check-write`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-050) **or** the honesty ledger says Unknown / Planned until probed. Same VFS `write`. Not POSIX. Not FAT32. This mile alone ≠ the ADR-048 product claim (Verified separately under ADR-048/052).
- FAT16 root readdir (`fat: readdir` / `fat: entries`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-056) **or** the honesty ledger says Unknown / Planned until probed. Same thin VFS. Not POSIX `getdents`. Not FAT32. This mile alone ≠ the ADR-048 product claim (Verified separately under ADR-048/052).
- FAT16 root delete (`fat: delete`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-057) **or** the honesty ledger says Unknown / Planned until probed. Same thin VFS. Not POSIX `unlink`. Not FAT32. This mile alone ≠ the ADR-048 product claim (Verified separately under ADR-048/052).
- Thin VFS prefix mounts (`vfs: mount` / `vfs: mounts`) are observable on serial and/or `#[test_case]` (NFR-10 / ADR-058) **or** the honesty ledger says Unknown / Planned until probed. Same thin VFS. Not POSIX `mount(2)`. Not a new disk FS. This mile alone ≠ the ADR-048 product claim (Verified separately under ADR-048/052).
