...
- Thin VFS + memfs (`fs: create` / `fs: write` / `fs: read` / `fs: el0` / `fs: ok`) is observable on serial and/or `#[test_case]` (NFR-10 / ADR-027 / Track A A6) **or** the honesty ledger says Unknown / Planned until probed. Not POSIX. Not FAT. App hosting stays Planned.
