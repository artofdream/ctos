# In-tree sample recipes (Track A / A8 + ADR-059)
...
`hello-libctos` does **not** call `fs_*`. `fs-libctos` does (create/open/read/write/close on `/memdemo` → memfs via ADR-058 `/mem` prefix). The kernel `/eprobe` trampoline remains a separate Verified EL0 VFS trip.
