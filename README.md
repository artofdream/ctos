...
```bash
./scripts/qemu-smoke.sh   # build + hello + paging + heap + timer tick + injected UART RX + BRK + fatal nested serial + cargo test + force-fail
cargo test                # #[test_case] including VBAR/BRK/stacks/timer/empty RX/MMU/frames/heap; QEMU exits 0 via ARM semihosting
```
